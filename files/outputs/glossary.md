# Monta Glossary (AI-Ready)

This glossary maps Monta terminology for AI models. Each entry includes description, usage rules, and translations.

## AC
**Tag:** industry

**Description:** Type of charge point with slower charging speeds, typically found in home, workplace, or public settings

**Translations:**
- DA: normalladning (AC)
- DE: Wechselstrom (AC)
- EN: AC
- EN_US: AC
- ES: AC
- FR: CA
- FR_CA: CA
- IT: CA
- NL: wisselstroom (AC)
- NO: AC
- RO: AC
- SV: AC

---

## AFIR 
**Tag:** industry

**Description:** Acronym for Alternative Fuels Infrastructure Regulation (AFIR) . Source and target capitalization intact.

**Translations:**
- DA: AFIR
- DE: AFIR
- EN: AFIR
- EN_US: AFIR
- ES: RICA
- FR: RICA
- FR_CA: AFIR
- IT: RICA
- NL: AFIR
- NO: AFIR
- RO: AFIR
- SV: AFIR

---

## API
**Tag:** product

**Description:** Monta’s Public API allows end customers to integrate their Monta-connected charge points and EVs with any system to use the data. For example, triggering a home charge based on conditions like house meter or solar energy

**Translations:**
- DA: API
- DE: API
- EN: API
- EN_US: API
- ES: API
- FR: API
- FR_CA: API
- IT: API
- NL: API
- NO: API
- RO: API
- SV: API

---

## API key
**Tag:** industry

**Description:** A unique identifier, that  serves as a "password" to validate the client's identity when making requests to the API.

**Translations:**
- DE: API-Schlüssel
- EN: API key
- EN_US: API key
- ES: clave de API
- FR: clé API
- FR_CA: clé API
- IT: chiave API
- NL: API-sleutel
- NO: API-nøkkel
- RO: cheie API
- SV: API-nyckel

---

## Administrator
**Tag:** product

**Description:** Role access functionality for Business plan users in HUB, enabling to diversify access permissions for team members within an account. Admin account has a full access and full functionaltity available. 

**Translations:**
- DA: Administrator
- DE: Admin
- EN: Administrator
- EN_US: Administrator
- ES: Administrador
- FR: Administrateur
- FR_CA: administrateur
- IT: Amministratore
- NO: Admin
- RO: Admin
- SV: Administratör

---

## Advenir
**Tag:** industry

**Description:** French government program aimed at expanding the nation's EV charging infrastructure. 

**Translations:**
- DA: Advenir
- DE: Advenir
- EN: Advenir
- EN_US: Advenir
- ES: Advenir
- FR: Advenir
- FR_CA: Advenir
- IT: Advenir
- NL: Advenir
- NO: Advenir
- RO: Advenir
- SV: Advenir

---

## Alternative Fuels Infrastructure Regulation
**Tag:** industry

**Description:** Refers to setting mandatory targets for EU countries to deploy infrastructure for alternative fuels, ensuring compatibility, user information, and payment options.

**Translations:**
- DA: Forordning om infrastruktur for alternative brændstoffer (AFIR)
- DE: Verordnung über die Infrastruktur für alternative Kraftstoffe (AFIR)
- EN: Alternative Fuels Infrastructure Regulation
- EN_US: Alternative Fuels Infrastructure Regulation
- ES: Reglamento de Infraestructuras de Combustibles Alternativos (RICA)
- FR: Règlement sur le déploiement d’une infrastructure pour carburants alternatifs
- FR_CA: Règlement sur le déploiement d’une infrastructure pour carburants alternatifs
- IT: Regolamento sulle Infrastrutture per i Carburanti Alternativi (RICA)
- NL: Verordening inzake infrastructuur voor alternatieve brandstoffen (AFIR)
- NO: Forordning om infrastruktur for alternative drivstoffer (AFIR)
- RO: Regulamentul privind infrastructura pentru combustibili alternativi (AFIR)
- SV: Förordning om infrastruktur för alternativa bränslen (AFIR)

---

## App Store
**Tag:** industry

**Description:** Apple's app marketplace for iOS, IpadOS. Source and target capitalization intact.

**Translations:**
- DA: App Store
- DE: App Store
- EN: App Store
- EN_US: App Store
- ES: App Store
- FR: App Store
- FR_CA: App Store
- IT: App Store
- NL: App Store
- NO: App Store
- RO: App Store
- SV: iTunes Store

---

## App clips
**Tag:** industry

**Description:** Description:
Lightweight Apple experience that lets iOS drivers start a charge or complete a quick action in Monta Charge without installing the full app.

Usage rules:
- Use “App Clips” (Apple’s official term).
- Translatable: Partial - FR “Extrait d’app”, FR_CA “Extrait d’appli”.
- Keep capitalization: Both words capitalized.
- Mention only when the experience differs from the full app.
- Do not equate with Shortcuts, Widgets, or Instant Apps.
- For Android, reference the equivalent “Instant Apps”.

Surface: Charge app (consumer).
Audience: EV drivers using iOS.
Tone: Technical-neutral.
See also: Instant Apps, Charge.

**Translations:**
- DA: App Clips
- DE: App Clips
- EN: App clips
- EN_US: App clips
- ES: App Clips
- FR: Extrait d’app
- FR_CA: Extrait d'appli
- IT: App Clips
- NL: Appclips
- NO: Appklipp
- RO: App Clips
- SV: App Clips

---

## Auto withdrawal
**Tag:** product

**Description:** Label of sub-header in Wallet > Withdraw section. Enables automatic monthly transfer of available funds to the user's bank account.

**Translations:**
- DA: automatisk udbetaling
- DE: Automatische Auszahlung
- EN: auto withdrawal
- EN_US: Auto withdrawal
- ES: transferencia automática	
- FR: retrait automatique
- FR_CA: retrait automatique
- IT: bonifico automatico
- NL: automatische uitbetaling
- NO: automatisk utbetaling
- RO: retragere automată
- SV: automatisk utbetalning	

---

## AutoCharge
**Tag:** product

**Description:** Source and target capitalization intact.  B2C feature enabling automatic charging. 

**Translations:**
- DA: AutoCharge
- DE: AutoCharge
- EN: AutoCharge
- EN_US: AutoCharge
- ES: AutoCharge
- FR: AutoCharge
- FR_CA: AutoCharge
- IT: Ricarica Automatica
- NL: AutoCharge
- NO: AutoCharge
- RO: AutoCharge
- SV: AutoCharge

---

## AutoStart
**Tag:** product

**Description:** Source and target capitalization intact. Feature enables to start automatic charge, when EV gets plugged into to charge point that is set to be Private.

**Translations:**
- DA: AutoStart
- DE: AutoStart
- EN: AutoStart
- EN_US: AutoStart
- ES: Inicio automático
- FR: Démarrage automatique
- FR_CA: AutoStart
- IT: Avvio automatico
- NL: AutoStart
- NO: AutoStart
- RO: AutoStart
- SV: AutoStart

---

## Basic
**Tag:** brand

**Description:** Source and target capitalization intact. Subscription plan at Monta.

**Translations:**
- DA: Basic
- DE: Basic
- EN: Basic
- EN_US: Basic
- ES: Basic
- FR: Basic
- FR_CA: De base
- IT: Basic
- NL: Basic
- NO: Basic
- RO: Basic
- SV: Basic

---

## CP
**Tag:** industry

**Description:** Charge Point (CP): Industry acronym for a charge point. Opt for the full term "charge point" or "charger" for EN-US

**Translations:**
- DA: ladestander
- DE: B2B: Ladestation; B2C: Wallbox
- EN: CP
- EN_US: CP
- ES: punto de recarga
- FR: borne de recharge
- FR_CA: borne de recharge
- IT: punto di ricarica
- NL: laadpaal
- NO: ladepunkt
- RO: stație de încărcare
- SV: laddpunkt

---

## CPI
**Tag:** industry

**Description:** Acronym for Charge Point Integration, enabling communication and data exchange between charging stations and central management systems or external platforms.

**Translations:**
- DA: ladestanderintegration
- DE: CPI
- EN: CPI
- EN_US: CPI
- ES: CPI
- FR: intégration de borne de recharge
- FR_CA: intégration de borne de recharge
- IT: CPI
- NL: CPI
- NO:  ladepunktsintegrasjon
- RO: CPI
- SV: laddpunktsintegration

---

## CPMS
**Tag:** industry

**Description:** Acronym for Charge Point Management System, e.g. HUB.

**Translations:**
- DA: CPMS
- DE: CPMS
- EN: CPMS
- EN_US: CSMS
- ES: CPMS
- FR: CPMS (Système de gestion de bornes de recharge)
- FR_CA: Système de gestion des bornes de recharge
- IT: CPMS
- NL: CPMS
- NO: CPMS
- RO: SGSÎ (Sistemul de Gestionare al Stației de Încărcare)
- SV: CPMS

---

## CPO
**Tag:** industry

**Description:** Abbrev for charge charge point operator, an entity responsible for the operation, maintenance, and technical management of charge points and sites.

**Translations:**
- DA: CPO
- DE: CPO
- EN: CPO
- EN_US: CPO
- ES: CPO
- FR: CPO
- FR_CA: CPO
- IT: CPO
- NL: CPO
- NO: CPO
- RO: CPO
- SV: CPO

---

## CT clamp
**Tag:** industry

**Description:** Current transformer clamp is an electrical device used to measure the current flowing through a conductor without having to make direct contact with the electrical circuit. Used in power monitoring, energy metering, and electrical diagnostics.

**Translations:**
- DA: strømsensor
- DE: Stromzange
- EN: CT clamp
- EN_US: CT clamp
- ES: pinza CT
- FR: pince CT
- FR_CA: pince ampèremétrique
- IT: pinza T.A.
- NL: CT-klem
- NO: CT klemme
- RO: clemă CT
- SV: CT-klämma

---

## California Pricing
**Tag:** industry

**Description:** California Pricing is an OCPP-based method for a Charge Point Management System (CPMS) to communicate pricing (price per kWh, running cost, final cost, etc.) to the charge point in real time during a charging session. This allows the charger’s display to show the current price and accumulated cost to the EV driver, fulfilling the California regulatory mandate for transparent pricing. The term “California Pricing” comes from its origin in California regulations, but the concept has since gained wider usage beyond California as a best practice for real-time tariff display and cost calculation in EV charging. It is not simply a descriptive phrase, but the informal name of a specific technical solution (defined by an OCA whitepaper) to a industry-wide problem. Usage rule: When mentioning for the first time, always expand the abbreviation.

**Translations:**
- DA: California Pricing (OCPP-udvidelse til realtime prisvisning)
- DE: California Pricing (OCPP-Erweiterung für Preisübermittlung in Echtzeit)
- EN: California Pricing
- EN_US: California Pricing
- ES: California Pricing (extensión OCPP para tarificación en tiempo real)
- FR: California Pricing (extension OCPP pour la tarification en temps réel)
- FR_CA: California Pricing (extension OCPP pour tarification en temps réel)
- IT: California Pricing (estensione OCPP per tariffazione in tempo reale)
- NL: California Pricing (OCPP-uitbreiding voor realtime prijsoverdracht)
- NO: California Pricing (OCPP-utvidelse for sanntids prisoverføring)
- RO: California Pricing (extensie OCPP pentru tarifare în timp real)
- SV: California Pricing (OCPP-tillägg för realtids prissättning)

---

## Charge
**Tag:** brand

**Description:** A proper noun. Monta consumer app.

**Translations:**
- DA: Monta Charge
- DE: Monta Charge
- EN: Monta Charge
- EN_US: Monta Charge
- ES: Monta Charge
- FR: Monta Charge
- FR_CA: Monta Charge
- IT: Monta Charge
- NL: Monta Charge
- NO: Monta Charge
- RO: Monta Charge
- SV: Monta Charge

---

## Chargers
**Tag:** product

**Description:** Description:
UI label on the Charge app home screen linking to the list or map of available charging locations.

Usage rules:
- Always Title Case: “Chargers.”
- Use plural only.
- Do not replace with “Charge points,” “Stations,” or “Sites.”
- Keep label to one word.

Surface: Charge app (Home screen).
Audience: EV drivers.
Tone: Product-neutral.
See also: Charge point, Charging site.

**Translations:**
- DA: Ladere
- DE: Ladestationen
- EN: Chargers
- EN_US: Chargers
- ES: Cargadores
- FR: Bornes
- FR_CA: Bornes
- IT: Punti di ricarica
- NL: Laadpalen
- NO: Ladere
- RO: Stații
- SV: Laddare

---

## DC
**Tag:** industry

**Description:** Type of charge point using direct current (DC), offering much higher power outputs. DC offer fast or ultra-fast charging, in contrary to AC. DC are usually found in charging parks, located in strategically by highways, rest areas, etc.

**Translations:**
- DA: lynladning (DC)
- DE: Gleichstrom (DC)
- EN: DC
- EN_US: DC
- ES: DC
- FR: CC
- FR_CA: CC
- IT: CC
- NL: gelijkstroom (DC)
- NO: DC
- RO: DC
- SV: DC

---

## DSO
**Tag:** industry

**Description:** See the full-term - distribution system operator (DSO) - for more details. 

**Translations:**
- DA: DSO
- DE: VNB
- EN: DSO
- EN_US: EP
- ES: DSO
- FR: GRD
- FR_CA: GRD
- IT: DSO
- NL: distributienetbeheerder (DNB)
- NO: DSO
- RO: OD
- SV: systemansvarig för distributionssystem (DSO)

---

## Data pipelines
**Tag:** brand

**Description:** B2B feature. A built‑in Hub capability that lets operators discover, connect, and monitor managed data‑sharing integrations to external endpoints such as national access points (NAPs), AFIR reporting, rebate programs, and similar compliance or partner systems. It automates delivery of required static and dynamic data and gives visibility into what’s shared and each connection’s status, all scoped by permissions

**Translations:**
- DA: Data Pipelines
- DE: Datenströme
- EN: Data Pipelines
- EN_US: Data Pipelines
- ES: Flujo de datos
- FR: Flux de données
- FR_CA: Flux de données
- IT: Flussi di dati
- NL: Data Pipelines
- NO: Data Pipelines
- RO: Fluxuri de date
- SV: Data Pipelines

---

## Data warehouse
**Tag:** product

**Description:** A feature/functionality for Monta to share data with partners.

**Translations:**
- DA: Data warehouse
- DE: Data warehouse
- EN: Data warehouse
- EN_US: Data warehouse
- ES: Data warehouse
- FR: Data warehouse
- FR_CA: Entrepôt de données partenaires
- IT: Data warehouse
- NL: Data warehouse
- NO: Data warehouse
- RO: Data warehouse
- SV: Data warehouse

---

## Data+
**Tag:** brand

**Description:** Label for product add-on that can be optionally purchased to unlock more advanced features for partner and operator subscriptions.

**Translations:**
- DA: Data+
- DE: Data+
- EN: Data+
- EN_US: Data+
- ES: Data+
- FR: Data+
- FR_CA: Données+
- IT: Data+
- NL: Data+
- NO: Data+
- RO: Data+
- SV: Data+

---

## EIN
**Tag:** industry

**Description:** Employer identification number - used for tax reporting purposes, including payroll, tax returns, etc. US-centric term.

**Translations:**
- DA: CVR-nummer
- DE: Steuer-Nr.
- EN: EIN
- EN_US: EIN
- ES: CIF/NIF
- FR: SIREN
- FR_CA: NE (numéro d’entreprise)
- IT: NIF 
- NL: BTW-nummer 
- NO: Org.nr.
- RO: NIF
- SV: Org.nr.

---

## EMSP
**Tag:** industry

**Description:** Abbr. for E-mobility Service Provider. Offers EV charging services to EV drivers via its own network or through roaming agreements with other networks. Used in the context of invoices and receipts.

**Translations:**
- DA: EMSP
- DE: EMSP
- EN: EMSP
- EN_US: EMSP
- ES: EMSP
- FR: EMSP
- FR_CA: eMSP
- IT: EMSP
- NL: EMSP
- NO: EMSP
- RO: EMSP
- SV: EMSP

---

## EV
**Tag:** industry

**Description:** Acronym for electric vehicles. Use local equivalent, either as an acronym or full term

**Translations:**
- DA: elbil
- DE: Elektroauto, Elektrofahrzeug
- EN: EV
- EN_US: EV
- ES: VE
- FR: VE
- FR_CA: VE
- IT: EV
- NL: EV / elektrisch voertuig / elektrische auto
- NO: elbil
- RO: VE (Vehicule Electrice)
- SV: elbil

---

## EVSE
**Tag:** industry

**Description:** Acronym for  Electric Vehicle Supply Equipment.  The physical charging equipment. Translation depends on the context:
1. Tech-facing - keep the acronym, used in technical context ( back-end, installer tools, documentation, API's etc.) 
2. User-facing - spell out acronym, use local equlavent. Used in B2C flows, marketing (not technical audience)

**Translations:**
- DA: EVSE or ladeudstyr
- DE: EVSE or Ladeeinrichtung
- EN: EVSE
- EN_US: EVSE or charging equipment
- ES: EVSE or equipo de recarga
- FR: EVSE or équipement de recharge
- FR_CA: EAVE or équipement d'alimentation pour véhicule électrique 
- IT: EVSE or attrezzatura di ricarica
- NL: EVSE or laadpunt
- NO: EVSE or ladeutstyr
- RO: EVSE or echipament de încărcare
- SV: EVSE or laddutrustning

---

## Enterprise
**Tag:** brand

**Description:** Source and target capitalization intact. Monta subscription plan, targetting large-scale charging across multiple sites and offering the complete user experience.

**Translations:**
- DA: Enterprise
- DE: Enterprise
- EN: Enterprise
- EN_US: Enterprise
- ES: Enterprise
- FR: Entreprise
- FR_CA: Entreprise
- IT: Enterprise
- NL: Enterprise
- NO: Enterprise
- RO: Enterprise
- SV: Enterprise

---

## Finance+
**Tag:** brand

**Description:** Label for product add-on that can be optionally purchased to unlock more advanced features for partner and operator subscriptions.

**Translations:**
- DA: Finance+
- DE: Finance+
- EN: Finance+
- EN_US: Finance+
- ES: Finance+
- FR: Finance+
- FR_CA: Finance+
- IT: Finance+
- NL: Finance+
- NO: Finance+
- RO: Finance+
- SV: Finance+

---

## Free
**Tag:** brand

**Description:** Source and target capitalization intact. Name of Monta's subscription plan, which is most limited and free of charge.

**Translations:**
- DA: Free
- DE: Free
- EN: Free
- EN_US: Free
- ES: Gratuito
- FR: Free
- FR_CA: Gratuit
- IT: Gratuito
- NL: Free
- NO: Free
- RO: Free
- SV: Free

---

## Growth+
**Tag:** brand

**Description:** Label for product add-on that can be optionally purchased to unlock more advanced features for partner and operator subscriptions.

**Translations:**
- DA: Growth+
- DE: Growth+
- EN: Growth+
- EN_US: Growth+
- ES: Growth+
- FR: Growth+
- FR_CA: Croissance+
- IT: Growth+
- NL: Growth+
- NO: Growth+
- RO: Growth+
- SV: Growth+

---

## Hardware Portal
**Tag:** brand

**Description:** Monta platform for hardware manufacturers to gain valuable, actionable insights on their charge points to enhance product quality and stability. The portal includes features like remote firmware updates and OCPP compatibility testing with Monta's OCPP Toolkit.

Loc details - DNT "Hardware" // "Portal" - follow existing Portal translation

**Translations:**
- DA: Hardware Portal
- DE: Hardware Portal
- EN: Hardware Portal
- EN_US: Hardware Portal
- ES: Portal de hardware
- FR: Portail Hardware
- FR_CA: Portail de l’équipement
- IT: Portale hardware
- NL: Hardware Portal
- NO: Maskinvareportal
- RO: Portal Hardware
- SV: Hardware-Portalen

---

## Help Centre
**Tag:** brand

**Description:** Publically available guide and help article database.

**Translations:**
- DA: Hjælpcenter
- DE: Support Center
- EN: Help Center
- EN_US: Help Center
- ES: Centro de ayuda
- FR: Centre d'assistance
- FR_CA: Centre d’aide
- IT: Centro assistenza
- NL: Helpcentrum
- NO: Hjelpesenter
- RO: Centru de asistență
- SV: Hjälpcenter

---

## Hub
**Tag:** product

**Description:** Charge point management system, enabling back-end solutions within EV operations, financial and charge point management. Key user segments - solution providers and large enterprises.

**Translations:**
- DA: Hub
- DE: Hub
- EN: Hub
- EN_US: Hub
- ES: Hub
- FR: Hub
- FR_CA: Hub
- IT: Hub
- NL: Hub
- NO: Hub
- RO: Hub
- SV: Hub

---

## Hypercare
**Tag:** brand

**Description:** Source and target capitalization intact. Service within Enterprise plan. Tailored support package for new enterprise customers to succeed with Monta.

**Translations:**
- DA: Hypercare
- DE: Hypercare
- EN: Hypercare
- EN_US: Hypercare
- ES: Hypercare/Soporte de inicio
- FR: Support au lancement
- FR_CA: Soutien intensif
- IT: Supporto al lancio
- NL: Hypercare
- NO: Hypercare
- RO: Suport Hypercare
- SV: Hypercare/lanseringsstöd

---

## IBAN
**Tag:** industry

**Description:** International Bank Account Number - standardized format for bank account numbers that is used internationally to facilitate the processing of cross-border transactions

**Translations:**
- DA: IBAN
- DE: IBAN
- EN: IBAN
- EN_US: routing number and account number
- ES: IBAN
- FR: IBAN
- FR_CA: IBAN
- IT: IBAN
- NL: IBAN
- NO: IBAN
- RO: IBAN
- SV: IBAN

---

## ICE vehicle
**Tag:** industry

**Description:** Acronym refers to 'Internal Combustion Engine vehicle', i.e. vehicles powered by fossil fuels (e.g., gasoline or diesel), as opposed to electric vehicles (EVs).

**Translations:**
- DA: konventionelt køretøj
- DE: Verbrenner
- EN: ICE vehicle
- EN_US: ICE vehicle
- ES: vehículo térmico
- FR: véhicule thermique
- FR_CA: véhicule thermique
- IT: veicolo termico
- NL: brandstofauto
- NO: fossilbil
- RO: mașină pe benzină sau motorină
- SV: fossildrivet fordon

---

## In-Between Charges
**Tag:** brand

**Description:** Title of Monta's podcast. Available on Spotify. Source and target capitalization intact.

**Translations:**
- DA: In-Between Charges
- DE: In-Between Charges
- EN: In-Between Charges
- EN_US: In-Between Charges
- ES: In-Between Charges
- FR: In-Between Charges
- FR_CA: Sur la route or Entre deux charges
- IT: In-Between Charges
- NL: In-Between Charges
- NO: In-Between Charges
- RO: In-Between Charges
- SV: In-Between Charges

---

## Insights
**Tag:** product

**Description:**  'Insights'  section and button label in the 'Charge' app. Displays accumulated data during charging and forecasts, such as SmartCharge, Spotprice, etc

**Translations:**
- DA: Indsigt
- DE: Reports
- EN: Insights
- EN_US: Insights
- ES: Información
- FR: Statistiques
- FR_CA: Analyses
- IT: Informazioni
- NL: Inzichten
- NO: Innsikt
- RO: Informații
- SV: Översikt

---

## Instant App
**Tag:** industry

**Description:** 
Lightweight app version for Android devices, enabling charging with the Charge app without having to download the app. The Apple version is called App clips. Technical term and less likely to be used for user-facing content. 


**Translations:**
- DA: Instant App
- DE: Instant App
- EN: Instant App
- EN_US: Instant App
- ES: Instant App
- FR: Appli instantanée
- FR_CA: Appli instantanée
- IT: Instant App
- NL: Instant App
- NO: Instant App
- RO: Aplicație instantanee
- SV: Instant App

---

## MID
**Tag:** industry

**Description:** Measurement Index Directive (MID) is a European Union regulation that sets standards for the accuracy and reliability of measurement instruments. In non-English languages, MID is typically retained as MID, used in formal, regulatory, or detailed documentation. The spelled-out term is sometimes used for clarification in long-form and content for less-technical segment. 

**Translations:**
- DA: MID
- DE: MID
- EN: MID
- EN_US: MID
- ES: MID
- FR: MID
- FR_CA: DIM (Directive sur les instruments de mesure) 
- IT: MID
- NL: MID
- NO: MID
- RO: MID
- SV: MID

---

## Made to Move
**Tag:** brand

**Description:** A global tagline. Always remains in English across all the languages

**Translations:**
- DA: Made to Move
- DE: Made to Move
- EN: Made to Move
- EN_US: Made to Move
- ES: Made to Move
- FR: Made to Move
- FR_CA: Made to Move
- IT: Made to Move
- NL: Made to Move
- NO: Made to Move
- RO: Made to Move
- SV: Made to Move

---

## Manager
**Tag:** product

**Description:** Role access functionality for Business plan users in HUB, enabling to diversify access permissions for team members within an account.  Manager role gives almost the same access rights as Admin, with few restrictions.

**Translations:**
- DA: Manager
- DE: Manager
- EN: Manager
- EN_US: Manager
- ES: Gestor
- FR: Manager
- FR_CA: gestionnaire
- IT: Manager
- NL: Manager
- NO: Manager
- RO: Manager
- SV: Ansvarig

---

## Map
**Tag:** product

**Description:** Label of "Map feature" displayed in the main landing UI in the app. Source and target capitalization intact.

**Translations:**
- DA: Kort
- DE: Karte
- EN: Map
- EN_US: Map
- ES: Mapa
- FR: Carte
- FR_CA: Carte
- IT: Mappa
- NL: Kaart
- NO: Kart
- RO: Hartă
- SV: Karta

---

## Me
**Tag:** product

**Description:** Label of "Me" button in the main landing UI in the app. Source and target capitalization intact.

**Translations:**
- DA: Mig
- DE: Ich
- EN: Me
- EN_US: Me
- ES: Perfil
- FR: Moi
- FR_CA: Moi
- IT: Profilo
- NL: Ik
- NO: Meg
- RO: Eu
- SV: Jag

---

## Monta Control
**Tag:** brand 

**Description:** Brand name for a secondary product offering integrated into the HUB, designed specifically for operators. It provides real-time access to raw OCPP logs, visibility into Plug & Charge sessions, diagnostic insights, and configuration capabilities for enhanced operational control and troubleshooting.

**Translations:**
- DA: Monta Control
- DE: Monta Control
- EN: Monta Control
- EN_US: Monta Control
- ES: Monta Control
- FR: Monta Control
- FR_CA: Monta Contrôle
- IT: Monta Control
- NL: Monta Control
- NO: Monta Control
- RO: Monta Control
- SV: Monta Control

---

## NFC
**Tag:** industry

**Description:** Used in the context of charge key.

**Translations:**
- DA: NFC
- DE: NFC
- EN: NFC
- EN_US: NFC
- ES: NFC
- FR: NFC
- FR_CA: NFC
- IT: NFC
- NL: NFC
- NO: NFC
- RO: NFC
- SV: NFC

---

## NOC Agent
**Tag:** brand

**Description:** B2B feature. AI-powered diagnostic assistant built into Monta Control and Hub
Reads session logs and related signals across multiple data sources to identify likely root causes of failed or degraded charging sessions, then presents clear, actionable explanations for operators and support teams

**Translations:**
- DA: NOC-agent
- DE: NOC-Agent
- EN: NOC Agent
- EN_US: NOC Agent
- ES: Agente NOC
- FR: Agent NOC
- FR_CA: Agent NOC
- IT: Agente NOC
- NL: NOC-agent
- NO: NOC-agent
- RO: Agent NOC
- SV: NOC-agent

---

## OCPI
**Tag:** industry

**Description:** Acronym for 'Open Charge Point Interface'. In user-facing content, OCPI is typically kept as an acronym rather than spelled out. Used mainly in technical contexts. It is an established and widely understood within the industry.

**Translations:**
- DA: OCPI
- DE: OCPI
- EN: OCPI
- EN_US: OCPI
- ES: OCPI
- FR: OCPI
- FR_CA: OCPI
- IT: OCPI
- NL: OCPI
- NO: OCPI
- RO: OCPI
- SV: OCPI

---

## OCPI tariff 
**Tag:** industry

**Description:** A standardized method for defining and sharing pricing information for electric vehicle (EV) charging services between various participants in the EV ecosystem. 

**Translations:**
- DA: OCPI-takst
- DE: OCPI-Tarif
- EN: OCPI tariff
- EN_US: OCPI rate
- ES: tarifa OCPI
- FR: tarif OCPI
- FR_CA: tarif OCPI
- IT: tariffa OCPI
- NL: OCPI-tarief
- NO: OCPI-tariff
- RO: tarif OCPI
- SV: OCPI-tariff

---

## OCPP
**Tag:** industry

**Description:** Acronym for 'Open Charge Point Protocol'. Technical standard for 'How charge points (hardware) communicate with EV software. In user-facing content, OCPP is typically kept as an acronym rather than spelled out. It is an established and widely understood within the industry.

**Translations:**
- DA: OCPP
- DE: OCPP
- EN: OCPP
- EN_US: OCPP
- ES: OCPP
- FR: OCPP
- FR_CA: OCPP
- IT: OCPP
- NL: OCPP
- NO: OCPP
- RO: OCPP
- SV: OCPP

---

## OCPP Toolkit
**Tag:** brand

**Description:** Monta Integration Testing tool, enabling manufacturers to test OCPP implementation of their charge point and firmware. This ensures stability and quality of the hardware, when used together with Monta's software.

**Translations:**
- DA: OCPP Toolkit
- DE: OCPP-Toolkit
- EN: OCPP Toolkit
- EN_US: OCPP Toolkit
- ES: Kit de herramientas de OCPP
- FR: Boîte à outils OCPP
- FR_CA: Boîte à outils OCPP
- IT: Kit di strumenti OCPP
- NL: OCCP-toolkit
- NO: OCPP-verktøy
- RO: Set de instrumente OCPP
- SV: OCPP-verktyg

---

## OICP
**Tag:** industry

**Description:** Acronym for 'Open InterCharge Protocol'. Technical standard how networks communicate with charge points. In user-facing content, OICP is typically kept as an acronym rather than spelled out. It is an established and widely understood within the industry.

**Translations:**
- DA: OICP
- DE: OICP
- EN: OICP
- EN_US: OICP
- ES: OICP
- FR: OICP
- FR_CA: OICP
- IT: OICP
- NL: OICP
- NO: OICP
- RO: OICP
- SV: OICP

---

## Open Charge Point Protocol
**Tag:** industry

**Description:** Spelled out term for OCPP. Technical and regulated term, remains in English in all languages

**Translations:**
- DA: Open Charge Point Protocol
- DE: Open Charge Point Protocol
- EN: Open Charge Point Protocol
- EN_US: Open Charge Point Protocol
- ES: Protocolo abierto de punto de recarga
- FR: protocole OCPP
- FR_CA: protocole OCPP
- IT: protocollo OCPP
- NL: Open Charge Point Protocol
- NO: Open Charge Point Protocol
- RO: Protocol Punct de Încărcare Deschis (OCPP)
- SV: Open Charge Point Protocol

---

## P2P
**Tag:** industry

**Description:** Facilitate direct charging transactions, when one private individual (the seller) allows another (the buyer) to charge an EV at their charge point, and payment is made directly from the buyer to the seller using the Monta platform. Used in invoiced.

**Translations:**
- DA: peer-to-peer
- DE: peer-to-peer
- EN: P2P
- EN_US: peer-to-peer
- ES: peer-to-peer
- FR: pair à pair
- FR_CA: peer-to-peer
- IT: peer-to-peer
- NL: peer-to-peer
- NO: peer-to-peer
- RO: peer-to-peer
- SV: peer-to-peer

---

## Plug&Charge
**Tag:** brand

**Description:** Feature enables instant charging upon cable connection, as an alternative to RFID and app authentication, for Monta-operated Charge Points.

**Translations:**
- DA: Plug&Charge
- DE: Plug&Charge
- EN: Plug&Charge
- EN_US: Plug&Charge
- ES: Plug&Charge
- FR: Plug&Charge
- FR_CA: Plug&Charge
- IT: Collega&Ricarica
- NL: Plug&Charge
- NO: Plug&Charge
- RO: Plug&Charge
- SV: Plug&Charge

---

## PowerBank
**Tag:** brand

**Description:** Grid service feature that connects charging points to the grid system, allowing Monta to pause and resume charging based on the grid’s imbalance at any given moment.

**Translations:**
- DA: PowerBank
- DE: PowerBank
- EN: PowerBank
- EN_US: PowerBank
- ES: PowerBank
- FR: PowerBank
- FR_CA: PowerBank
- IT: PowerBank
- NL: PowerBank
- NO: PowerBank
- RO: PowerBank
- SV: PowerBank

---

## Pro
**Tag:** brand

**Description:** Subscription plan, targetting smaller sites that need to manage multiple charge points. Source and target capitalization intact.

**Translations:**
- DA: Pro
- DE: Pro
- EN: Pro
- EN_US: Pro
- ES: Pro
- FR: Pro
- FR_CA: Pro
- IT: Pro
- NL: Pro
- NO: Pro
- RO: Pro
- SV: Pro

---

## Professional 
**Tag:** brand

**Description:** A type of Team (feature), designed to cater  a variety of B2B needs, such as  Housing association, Parking, Hotel, and Company.

**Translations:**
- DA: Business
- DE: Business
- EN: Business
- EN_US: Business
- ES: Business
- FR: Business
- FR_CA: Business
- IT: Business
- NL: Business
- NO: Business
- RO: Business
- SV: Business

---

## RFID
**Tag:** industry

**Description:** See "Charge key" term, synonyms. Source and target capitalization intact.

**Translations:**
- DA: RFID-kort
- DE: RFID-Karte
- EN: RFID
- EN_US: RFID card
- ES: carjeta RFID
- FR: carte RFID
- FR_CA: carte RFID
- IT: carta RFID
- NL: RFID-kaart
- NO: RFID-kort
- RO: card RFID
- SV: RFID-kort

---

## Refer & Earn
**Tag:** brand

**Description:** Subheader title of referal label in 'Me' section in the mobile app. Enabling discount when a new user is being referred to Monta by an existing user. Source and target capitalization intact.

**Translations:**
- DA: Henvis og tjen
- DE: Empfehlen & Verdienen
- EN: Refer & Earn
- EN_US: Refer & Earn
- ES: Invita a un amigo
- FR: Parrainez et gagnez
- FR_CA: Référez et gagnez
- IT: Invita un amico
- NL: Verwijs en Verdien
- NO: Verv & tjen
- RO: Recomandă și câștigă
- SV: Värva & tjäna

---

## SEPA Direct Debit
**Tag:** industry

**Description:** A pan-European direct debit system that allows merchants and businesses to collect Euro-denominated payments from accounts in the 36 SEPA (Single Euro Payments Area) countries. Source and target capitalization intact.

**Translations:**
- DA: SEPA Direct Debit
- DE: SEPA-Lastschrift
- EN: SEPA Direct Debit
- EN_US: SEPA Direct Debit
- ES: Adeudo directo SEPA
- FR: Prélèvement SEPA
- FR_CA: Prélèvement automatique européen (SEPA)
- IT: Addebito diretto SEPA
- NL: SEPA-incasso
- NO: SEPA-avtalegiro
- RO: Debitare directă SEPA
- SV: SEPA Direct Debit

---

## SSO
**Tag:** industry

**Description:** Single sign-on. SSO is an authentication scheme that allows a user to log in with a single ID to any of several related, yet independent, software systems.

**Translations:**
- DA: SSO
- DE: SSO
- EN: SSO
- EN_US: SSO
- ES: SSO
- FR: SSO
- FR_CA: AU
- IT: SSO
- NL: SSO
- NO: SSO
- RO: SSO
- SV: SSO

---

## Shop
**Tag:** brand

**Description:** Shop feature, where Monta branded charge keys, RDIF, stickers are purchased.

**Translations:**
- DA: Shop
- DE: Shop
- EN: Shop
- EN_US: Shop
- ES: Tienda
- FR: Boutique
- FR_CA: Boutique
- IT: Negozio
- NL: Shop
- NO: Shop
- RO: Magazin
- SV: Butik

---

## Single Sign-On
**Tag:** industry

**Description:** Single Sign-On. SSO is an authentication scheme that allows a user to log in with a single ID to any of several related, yet independent, software systems.

**Translations:**
- DA: Single Sign-On
- DE: Single Sign-on
- EN: Single Sign-on
- EN_US: Single Sign-on
- ES: inicio de sesión único
- FR: authentification unique
- FR_CA: authentification unique
- IT: Single Sign-on
- NL: Single Sign-On
- NO: Single sign-on
- RO: Single sign-on
- SV: Samlad inloggning

---

## StationID
**Tag:** industry

**Description:** A unique identifier for a specific charge station. A technical term, used in technical and software documentations. 3rd party term Advenir

**Translations:**
- DA: StationID
- DE: Stations-ID
- EN: StationID
- EN_US: StationID
- ES: StationID
- FR: StationID
- FR_CA: StationID
- IT: StationID
- NL: StationID
- NO: StasjonsID
- RO: StationID
- SV: StationsID

---

## Stay in charge
**Tag:** brand

**Description:** Monthly newsletter's title

**Translations:**
- DA: Stay in charge
- DE: Stay in Charge
- EN: Stay in charge
- EN_US: Stay in charge
- ES: Stay in charge
- FR: Stay in charge
- FR_CA: Toujours en charge
- IT: Stay in charge
- NL: Stay in charge
- NO: Stay in charge
- RO: Stay in charge
- SV: Stay in charge

---

## Supporter
**Tag:** product

**Description:** Role access functionality for Business plan users in HUB, enabling to diversify access permissions for team members within an account. Supporter  role  is dedicated to the support team and handles end-users queries, troubleshooting issues with charge points.

**Translations:**
- DA: Supporter
- DE: Support-Mitarbeiter
- EN: Supporter
- EN_US: Supporter
- ES: Miembro de Soporte
- FR: Assistant
- FR_CA: assistant
- IT: Membro di supporto
- NO: Supporter
- RO: Supporter
- SV: Supportgivare

---

## VAT
**Tag:** industry

**Description:** The tax rate  in percentage applied to the price of goods or services. Used in the EU countries for cross-boarder sales.  US equivalence is sales tax.

**Translations:**
- DA: moms
- DE: USt
- EN: VAT
- EN_US: sales tax
- ES: IVA
- FR: TVA
- FR_CA: taxe de vente
- IT: IVA
- NL: BTW
- NO: MVA
- RO: TVA
- SV: moms

---

## VAT 
**Tag:** industry

**Description:** A VAT identification number (or VAT number) identifies a business registered for VAT, used for cross-border trade within the EU and internationally. Format - country code + numeric/alphanumeric ID

**Translations:**
- DA: momsnr.
- DE: USt-IdNr.
- EN: VAT
- EN_US: Tax ID
- ES: N.º de IVA
- FR: n° TVA
- FR_CA: NE (numéro d’entreprise)
- IT: P.IVA
- NL: BTW-nummer 
- NO: MVA-nr.
- RO: cod TVA
- SV: momsnr

---

## VAT export 
**Tag:** industry

**Description:** EU VAT regulation term, applied in use cases, where 0% VAT rate is applied. Zero-rated VAT must be reported appropriately in transactions.

**Translations:**
- DA: momsfritagelse ved eksport
- DE: Ausfuhrlieferung (0 % MwSt)
- EN: VAT export 
- EN_US: zero-rated sales tax (EU VAT rules apply)
- ES: exportación exenta de IVA
- FR: exportation exonérée de TVA
- FR_CA: taxe de vente à taux zéro (règles de la TVA de l’UE applicables)
- IT: operazioni non imponibili 
- NL: uitvoerlevering (0 % btw)
- NO: MVA-fritak ved eksport	
- RO: export scutit de TVA
- SV: momsfri export

---

## VIN
**Tag:** industry

**Description:** Refers to the Vehicle Identification Number (VIN), a unique code that identifies a vehicle and provides details about its manufacturing. "VIN" is globally standardized and commonly used across languages. For clarity in long-form content, it can be spelled out with local translation

**Translations:**
- DA: stelnummer
- DE: FIN
- EN: VIN
- EN_US: VIN
- ES: VIN/Número de Identificación Vehicular
- FR: numéro VIN
- FR_CA: code VIN 
- IT: numero di identificazione del veicolo (VIN)
- NL: VIN / Voertuigidentificatienummer
- NO: chassisnummer
- RO: VIN
- SV: VIN

---

## account
**Tag:** product

**Description:** Refers to an account in the portal/app; not bank account

**Translations:**
- DA: konto
- DE: Account
- EN: account
- EN_US: account
- ES: cuenta
- FR: compte
- FR_CA: compte
- IT: account
- NL: account
- NO: konto
- RO: cont
- SV: konto

---

## additional fee
**Tag:** product

**Description:** Extra fees, a Team owner can apply to a price group, i.e. start fee, charging fee, minute fee & idle fee.

**Translations:**
- DA: ekstragebyr
- DE: zusätzliche Gebühr
- EN: additional fee
- EN_US: additional fee
- ES: tasa adicional
- FR: frais supplémentaires
- FR_CA: frais supplémentaires
- IT: costo supplementare
- NL: aanvullende kosten
- NO: tilleggsgebyr
- RO: taxă adițională
- SV: extra avgift

---

## alert
**Tag:** product

**Description:** A feature to provide operators and solution providers with a structured way to monitor and respond to important system or operational events. 
event-driven notifications raised when conditions are met (e.g., device offline, error state, repeated failed charges). They’re the building blocks that surface issues to operators via our notification system and UI. Linked to the functionality "Alert group"

**Translations:**
- DA: advarsel
- DE: Warnung
- EN: alert
- EN_US: alert
- ES: alerta
- FR: alerte
- FR_CA: alerte
- IT: avvisi
- NL: waarschuwing
- NO: varsel
- RO: alertă
- SV: varning

---

## alert group
**Tag:** product

**Description:** A feature for configuring notifications about critical incidents on charge points, charging sites, ensuring users are alerted about the incidents.

**Translations:**
- DA: advarselsgruppe 
- DE: Warngruppe
- EN: alert group
- EN_US: alert group
- ES: grupo de alertas
- FR: groupe d’alertes
- FR_CA: groupe d’alertes
- IT: gruppo di avvisi
- NL: waarschuwingsgroep
- NO: varslingsgruppe
- RO: grup de alerte
- SV: varningsgrupp

---

## auto refill
**Tag:** product

**Description:** Label of a sub-header in Wallet > Refill section. It is an automatic top-up functionality in Wallet. Users can enable the auto-refill option and adjust what amount is being auto-refilled into the Wallet, when the balance is nearing zero.

**Translations:**
- DA: automatisk optankning
- DE: automatisches Aufladen
- EN: auto refill
- EN_US: auto refill
- ES: recarga automática / relleno automático
- FR: recharge automatique
- FR_CA: dépôt automatique
- IT: ricarica automatica
- NL: automatisch aanvullen
- NO: automatisk påfylling
- RO: realimentare automată
- SV: autopåfyllning

---

## automatic reading
**Tag:** industry

**Description:** Remote process of automatically collecting data from an electricity meter, where consumption information is recorded and stored for remote access by the utility or service provider

**Translations:**
- DA: automatisk aflæsning
- DE: Automatische Zählerablesung
- EN: automatic reading
- EN_US: automatic reading
- ES: lectura automática
- FR: relevé automatique
- FR_CA: lecture automatique
- IT: lettura automatica
- NL: automatisch uitlezen
- NO: automatisk avlesning
- RO: citire automată
- SV: automatisk avläsning

---

## available
**Tag:** product

**Description:** Adj. refers to charge point status, is integrated and available for a charge

**Translations:**
- DA: ledig
- DE: verfügbar
- EN: available
- EN_US: available
- ES: disponible
- FR: disponible
- FR_CA: disponible
- IT: disponibile
- NL: beschikbaar
- NO: ledig
- RO: disponibil
- SV: tillgänglig

---

## balance
**Tag:** product

**Description:** Noun. Available amount of money in the user's Wallet in consumer and business app.

**Translations:**
- DA: saldo
- DE: Saldo
- EN: balance
- EN_US: balance
- ES: saldo
- FR: solde
- FR_CA: solde
- IT: saldo
- NL: saldo
- NO: saldo
- RO: sold
- SV: saldo

---

## battery capacity
**Tag:** industry

**Description:** amount of electrical energy a battery can store and deliver, typically measured in ampere-hours (Ah) or watt-hours (Wh). It indicates the total charge the battery can hold and release over time.

**Translations:**
- DA: batterikapacitet
- DE: Batteriekapazität
- EN: battery capacity
- EN_US: battery capacity
- ES: capacidad de la batería
- FR: capacité de la batterie
- FR_CA: capacité de la batterie
- IT: capacità della batteria
- NL: batterijcapaciteit
- NO: batterikapasitet
- RO: capacitate baterie
- SV: batterikapacitet

---

## battery level
**Tag:** industry

**Description:** Label of a field that tells the current battery-% of an integrated car, i.e. "Battery level: 78%"

**Translations:**
- DA: batteriniveau
- DE: Batteriestand
- EN: battery level
- EN_US: battery level
- ES: nivel de batería
- FR: niveau de la batterie
- FR_CA: niveau de la batterie
- IT: livello della batteria
- NL: batterijniveau
- NO: batterinivå
- RO: nivel baterie
- SV: batterinivå

---

## bilateral agreement
**Tag:** product

**Description:** Feature in Monta Hub allowing operators to create and manage price agreements and charge point access with EMSPs (Electric Mobility Service Providers), typically used in roaming scenarios.

**Translations:**
- DA: bilateral aftale	
- DE: Bilaterale Vereinbarung
- EN: bilateral agreement
- EN_US: bilateral agreement
- ES: acuerdo bilateral
- FR: accord bilatéral 
- FR_CA: accord bilatéral 
- IT: accordo bilaterale
- NL: bilaterale overeenkomst
- NO: bilateral avtale	
- RO: acord bilateral
- SV: bilateralt avtal

---

## busy
**Tag:** product

**Description:** Adj. refers to the charge point state. Busy can entail various sub-states: blocked, scheduled, and charging.

**Translations:**
- DA: optaget
- DE: Nicht verfügbar
- EN: busy
- EN_US: busy
- ES: ocupado
- FR: occupée 
- FR_CA: occupée
- IT: occupato
- NL: niet beschikbaar
- NO: opptatt
- RO: ocupat
- SV: upptagen

---

## certified electronic mail
**Tag:** industry

**Description:** Italian billing compliance term, used in Italy-only for  legal communication purposes. No equlavent concepts in other languages, thus do a literal translation. 

**Translations:**
- DA: certificeret elektronisk post
- DE: Zertifizierte Elektronische Post
- EN: certified electronic mail
- EN_US: certified electronic mail
- ES: correo electrónico certificado
- FR: courrier électronique certifié
- FR_CA: courrier électronique certifié
- IT: posta elettronica certificata
- NL: gecertificeerde email
- NO: sertifisert elektronisk post
- RO: poștă electronică certificată
- SV: certifierad elektronisk post

---

## charge
**Tag:** industry 

**Description:** adjective (describing log, ID, object) 

**Translations:**
- DA: lade-
- DE: Lade-
- EN: charge
- EN_US: charge
- ES: de carga
- FR: de recharge
- IT: di ricarica
- NL: laad-
- NO: lade-
- RO: de încărcare
- SV: ladd-

---

## charge amount
**Tag:** industry

**Description:** Refers to amount of charged kWh during a charge session.

**Translations:**
- DA: lademængde
- DE: Lademenge
- EN: charge amount
- EN_US: charge amount
- ES: cantidad de carga
- FR: énergie chargée
- FR_CA: quantité d’énergie
- IT: quantità caricata
- NL: laadhoeveelheid
- NO: lademengde
- RO: număr de KWh
- SV: laddningsmängd

---

## charge discount
**Tag:** product

**Description:** A in-product term for the feature - Charge Subscription - where users can apply a discount on charging. B2B feature in HUB. Not capitalized.

**Translations:**
- DA: laderabat
- DE: Laderabatt
- EN: charge discount
- EN_US: charging discount
- ES: descuento en la carga
- FR: réduction de recharge
- FR_CA: rabais sur la recharge
- IT: sconto sulla ricarica 
- NL: laadkorting
- NO: lade-rabat
- RO: discount la Încărcare
- SV: laddningsrabatt

---

## charge history
**Tag:** product

**Description:** Label for the 'Charge History' header in the 'Me' section, showing charging logs with kWh, charge point name, and total price

**Translations:**
- DA: ladehistorik
- DE: Ladehistorie
- EN: charge History
- EN_US: charge history
- ES: historial de cargas
- FR: historique des recharges
- FR_CA: historique des recharges
- IT: cronologia delle ricariche
- NL: laadgeschiedenis
- NO: ladehistorikk
- RO: istoricul încărcărilor
- SV: laddningshistorik

---

## charge key
**Tag:** industry

**Description:** Description:
Physical device (RFID card or fob) used to identify a user or team and authorize charging on compatible charge points.

Usage rules:
- Use “charge key” in consumer-facing and support content.
- Use “RFID card” or “RFID tag” in technical or integration documentation.
- Lowercase in running text unless part of a label (“Charge Key Settings”).
- Distinguish from payment or access keys; this is an authentication device.

Surface: Charge app (setup), Hub (management), Help Center.
Audience: Drivers, operators, installers, support, partners.
Tone: Supportive-technical.
See also: RFID, Wallet, Team, Charge session.

**Translations:**
- DA: ladebrik
- DE: Ladekarte
- EN: charge key
- EN_US: charge key
- ES: tarjeta de carga
- FR: carte de recharge
- FR_CA: carte de recharge
- IT: tessera di ricarica
- NL: laadpas
- NO: ladebrikke
- RO: card de încărcare
- SV: laddbricka

---

## charge limit
**Tag:** industry

**Description:** The maximum level to which an electric vehicle's battery is charged during a charging session.

**Translations:**
- DA: ladegrænse
- DE: Ladegrenze
- EN: charge limit
- EN_US: charge limit
- ES: límite de carga
- FR: limite de charge
- FR_CA: limite de recharge
- IT: limite di carica
- NL: laadcapaciteit
- NO: ladegrense
- RO: limita de încărcare
- SV: laddningsgräns

---

## charge point
**Tag:** industry

**Description:** Default term, electrical outlet that supplies electrical power for charging plug-in electric vehicles. In copy sometimes used as a charger, or charging station. In target locale translate into the default term - charge point.

**Translations:**
- DA: ladestander
- DE: Ladestation
- EN: charge point
- EN_US: charger
- ES: punto de recarga
- FR: borne de recharge default term / borne -  only in space-constrained contexts 
- FR_CA: borne
borne de recharge
- IT: punto di ricarica
- NL: laadpaal
- NO: ladepunkt
- RO: stație de încărcare
- SV: laddpunkt

---

## charge point management system
**Tag:** industry

**Description:** Description:
Software platform used by operators to monitor, manage, and optimize EV charging infrastructure. Handles connectivity, pricing, access, reporting, and system integrations.

Usage rules:
- Use “charge point management system” in full for general copy.
- Use “CPMS” only in technical docs after introducing the full term once.
- Avoid “CSMS”; Monta standardizes on “CPMS.”
- When referring to Monta’s platform, use “Monta Hub.”

Surface: Docs, Partner integrations, Marketing.
Audience: Operators, enterprises, developers.
Tone: Technical-formal.
See also: Hub, Charge point, Operator.

**Translations:**
- DA: styresystem til ladestandere
- DE: Managementsystem für Ladestationen
- EN: charge point management system
- EN_US: charge station management system
- ES: sistema de gestión de puntos de recarga
- FR: système de gestion de borne de recharge
- FR_CA: système de gestion de borne de recharge
- IT: sistema di gestione dei punti di ricarica
- NL: laadpaalbeheersysteem
- NO: styresystem for ladepunkter
- RO: sistemul de gestionare al stației de încărcare
- SV: laddningslösning för elbilar

---

## charge point operator
**Tag:** industry

**Description:** A charge point operator (CPO) is an entity that owns, operates, and maintains charging stations for electric vehicles (EVs). Operators at Monta are considered as Partners. many functionalities are designed to support their needs.

**Translations:**
- DA: ladeoperatør
- DE: Ladestationsbetreiber
- EN: Rechnung
- EN_US: charger operator (UI) / turnkey provider (revorg)
- ES: operador de carga
- FR: opérateur de recharge
- FR_CA: opérateur d’infrastructure de recharge
- IT: operatore di ricarica
- NL: laadoperator
- NO: ladeoperatør
- RO: operator stație de încărcare
- SV: laddoperatör

---

## charge point settings
**Tag:** product

**Description:** Title of charge point settings in the 'Charge' app.

**Translations:**
- DA: indstillinger for ladestander
- DE: Einstellungen der Ladestation
- EN: charge point settings
- EN_US: charger settings
- ES: configuración del punto de recarga
- FR: paramètres de la borne de recharge
- FR_CA: paramètres de la borne de recharge
- IT: impostazioni del punto di ricarica
- NL: laadpaalinstellingen
- NO: ladepunktsinnstillinger
- RO: setări stație de încărcare
- SV: laddpunktsinställningar

---

## charge point subscription
**Tag:** product

**Description:** An add-on service for private charge point maintenance. Targeted for private CP owners and facilitated typically by charge point operators, and installers.

**Translations:**
- DA: ladestanderabonnement
- DE: Abonnement
- EN: charge point subscription
- EN_US: charger subscription
- ES: suscripción al punto de recarga
- FR: abonnement à la borne de recharge
- FR_CA: abonnement de borne
- IT: abbonamento al punto di ricarica
- NL: laadpaalabonnement
- NO: abonnement
- RO: abonament stații de încărcare
- SV: laddpunktsabonnemang

---

## charge power
**Tag:** industry

**Description:** Description:
Amount of electrical power delivered from the charge point to the EV at a given moment, measured in kilowatts (kW).

Usage rules:
- Use “charge power” for live or measured performance (telemetry, dashboards, analytics).
- Do not confuse with energy (kWh, total consumed).
- Format units as lowercase “kW” with a non-breaking space before the value (e.g., “11 kW”).
- In user-facing copy, prefer “charging speed” when describing perceived performance.

Surface: Hub dashboards, Charge session data, Technical docs.
Audience: Operators, installers, data teams.
Tone: Technical-precise.
See also: Charging speed, kW, Load balancing.

**Translations:**
- DA: ladeeffekt
- DE: Ladeleistung
- EN: charge power
- EN_US: charge power
- ES: potencia de carga
- FR: puissance de charge
- FR_CA: puissance de charge
- IT: potenza di carica
- NL: laadvermogen
- NO: ladeeffekt
- RO: putere de încărcare
- SV: laddeffekt

---

## charge price
**Tag:** industry

**Description:** Price accumulated during a charge session. It relates to member price, price per kWH, price per min.

**Translations:**
- DA: ladepris
- DE: Ladepreis
- EN: charge price
- EN_US: charge price
- ES: precio de carga
- FR: prix de la recharge
- FR_CA: prix de la recharge
- IT: prezzo della ricarica
- NL: laadprijs
- NO: ladepris
- RO: tarif încărcare
- SV: laddningspris

---

## charge profile
**Tag:** product

**Description:** Source and target capitalization intact. Business app feature (in Portal), enabling users to set up pre-set charge sessions, based either on SmartCharge preferences or preferred schedules. These can be applied for fleet and private charge sessions.

**Translations:**
- DA: ladeprofil
- DE: Ladeprofil
- EN: charge profile
- EN_US: charge profile
- ES: perfil de carga
- FR: profil de recharge
- FR_CA: profil de recharge
- IT: profilo di ricarica
- NL: laadprofiel
- NO: ladeprofil
- RO: profil de încărcare
- SV: laddningsprofil

---

## charge rate
**Tag:** brand

**Description:** Refers to the completed charging performance of an EV, linked to "charging stability," often represented as a percentage (e.g., 98% success rate, surpassing the industry standard of 85%)

**Translations:**
- DA: opladningssuccesrate
- DE: Ladezuverlässigkeit
- EN: charge rate
- EN_US: charge rate
- ES: rendimiento de carga
- FR: taux de réussite des sessions
- FR_CA: taux de recharge
- IT: tasso di ricarica
- NL: laadpercentage
- NO: ladepris
- RO: rată încărcare
- SV: genomförda laddningar

---

## charge session
**Tag:** industry

**Description:** The timespan in which an EV is being charged.

**Translations:**
- DA: ladesession
- DE: Ladevorgang
- EN: charge session
- EN_US: charge session
- ES: sesión de carga
- FR: session de recharge
- FR_CA: session de recharge
- IT: sessione di ricarica
- NL: laadsessie
- NO: ladeøkt
- RO: sesiune de încărcare
- SV: laddningssession

---

## charger
**Tag:** industry

**Description:** Charger (charge point): "Charger" refers to "charge point." 
While these terms may be used interchangeably in the source copy, ALWAYS use the target language translation for "Charge Point" when translating.

**Translations:**
- DA: ladestander
- DE: Ladestation
- EN: charger
- EN_US: charger
- ES: punto de recarga
- FR: borne de recharge
- FR_CA: borne de recharge
- IT: punto di ricarica
- NL: laadpaal
- NO: ladepunkt
- RO: stație de încărcare
- SV: laddpunkt

---

## charging
**Tag:** industry 

**Description:** adjective (describing process, states or attribute to the process

**Translations:**
- DA: lade-
- DE: Lade-
- EN: charging
- EN_US: charging
- ES: de carga
- FR: de recharge
- IT: di ricarica
- NL: laad-
- NO: lade-
- RO: de încărcare
- SV: ladd-

---

## charging app
**Tag:** industry

**Description:** Description:
Generic term for mobile apps that allow EV drivers to find, start, schedule, or pay for charging sessions.

Usage rules:
- Use “charging app” only when referring to the general category of EV charging software.
- Never use in UI; use “Charge” when referring to Monta’s product.
- Keep lowercase in running text.
- In marketing or educational content, may be used comparatively (e.g., “Unlike other charging apps…”).

Surface: Marketing, SEO content, documentation.
Audience: EV drivers, operators.
Tone: Neutral-informative.
See also: Charge, Hub.

**Translations:**
- DA: ladeapp
- DE: Lade-App
- EN: charging app
- EN_US: charging app
- ES: aplicación de recarga
- FR: application de recharge
- FR_CA: application de recharge
- IT: app di ricarica
- NL: laadapp
- NO: ladeapp
- RO: aplicație de încărcare
- SV: laddningsapp

---

## charging cable
**Tag:** industry

**Description:** Description:
A cable that connects an electric vehicle to a charge point to deliver electricity for charging.

Usage rules:
- Use “charging cable” in documentation, help content, and marketing.
- Use short form “Cable” in UI only when context makes the meaning clear.
- Do not abbreviate (avoid “EV cable” or “CP cable”).
- Differentiate from connector (plug end) and charge point (hardware unit).

Surface: Help Center, Docs, Installer workflows.
Audience: EV drivers, installers, support staff.
Tone: Technical-neutral.
See also: Connector, Charge point.

**Translations:**
- DA: ladekabel
- DE: Ladekabel
- EN: charging cable
- EN_US: charging cable
- ES: cable de carga
- FR: câble de recharge
- FR_CA: câble de recharge
- IT: cavo di ricarica






- NL: laadkabel
- NO: ladekabel
- RO: cablu de încărcare
- SV: laddkabel

---

## charging fee
**Tag:** product

**Description:** Fee type (visible to the user) A one-off fee to be applied after a specified time of charging. e.g. One-time fee of DKK 50,00 is added after the first 60 min. of charging

**Translations:**
- DA: ladegebyr
- DE: Ladegebühr
- EN: charging fee
- EN_US: charging fee
- ES: tasa de carga
- FR: frais de recharge
- FR_CA: frais de recharge
- IT: costo di ricarica
- NL: laadkosten
- NO: ladeavgift
- RO: taxă de încărcare
- SV: laddavgift

---

## charging point
**Tag:** industry

**Description:** Monta's default term is charge point and please use the corresponding translation, despite a synonym of charge point is used in the copy.

**Translations:**
- DA: ladestander
- DE: Ladestation
- EN: charging point
- EN_US: charger
- ES: punto de recarga
- FR: borne de recharge
- FR_CA: borne de recharge
- IT: punto di ricarica
- NL: laadpaal
- NO: ladepunkt
- RO: stație de încărcare
- SV: laddpunkt

---

## charging rule
**Tag:** product

**Description:** Charge schedule settings in the 'Charge' app allow users to pre-define charging schedules when private users plug in their cable into their own charge point. Automates  repetitive scheduling tasks for home charging car owners. 

**Translations:**
- DA: laderegel
- DE: Lade-Regel
- EN: charging rule
- EN_US: charging rule
- ES: regla de carga
- FR: règle de recharge
- FR_CA: règle de recharge
- IT: regola di ricarica
- NL: laadregel
- NO: laderegel
- RO: regulă de încărcare
- SV: laddningsregel 

---

## charging schedule
**Tag:** product

**Description:** A plan for charging an EV that lets owners customize charge times for efficiency and cost-effectiveness. In the Charge app, this is managed through the 'Schedule' feature, where users pre-set a charging time window.

**Translations:**
- DA: ladeplan
- DE: Ladeplan
- EN: charging schedule
- EN_US: charging schedule
- ES: programación de cargas
- FR: période de recharge
- FR_CA: période de recharge
- IT: programma di ricarica
- NL: laadschema
- NO: ladeplan
- RO: programare încărcare
- SV: laddningsschema

---

## charging site
**Tag:** industry

**Description:** A location where EVs charge, equipped with infrastructure and typically containing multiple charging units

**Translations:**
- DA: ladeplads
- DE: Standort
- EN: charging site
- EN_US: charging site
- ES: estación de carga
- FR: site de recharge
- FR_CA: site
site de recharge
- IT: stazione di ricarica
- NL: laadlocatie
- NO: ladested
- RO: locație de încărcare
- SV: laddplats

---

## charging speed
**Tag:** industry

**Description:** Description:
Rate at which an EV battery gains energy during charging, expressed as power (kW) or duration.

Usage rules:
- Use “charging speed” in consumer or marketing contexts (e.g., “fast charging”).
- Use “charge power (kW)” for technical data.
- Do not confuse charging speed (rate) with charging time (duration).
- Keep lowercase except in headers or feature names.

Surface: Marketing, Charge app, Help Center, Docs.
Audience: EV drivers, operators, support.
Tone: Informative-consumer.
See also: Charge power.

**Translations:**
- DA: ladehastighed
- DE: Ladegeschwindigkeit
- EN: charging speed
- EN_US: charging speed
- ES: velocidad de carga
- FR: vitesse de recharge
- FR_CA: vitesse de recharge
- IT: velocità di ricarica
- NL: laadsnelheid
- NO: ladehastighet
- RO: viteza de încărcare
- SV: laddningshastighet

---

## charging station
**Tag:** industry

**Description:** A charging station is a site with one or more charging points where EVs can be recharged. It includes chargers, communication systems for payment and monitoring, and may be publicly or privately accessible. Used in CPI Tool, Hardware Portal. 

**Translations:**
- DA: ladestation
- DE: Ladestation
- EN: charging station
- EN_US: charging station
- ES: estación de carga
- FR: station de recharge
- FR_CA: station de recharge
- IT: stazione di ricarica
- NL: laadstation
- NO: ladestasjon
- RO: stație de încărcare
- SV: laddstation

---

## chat
**Tag:** product

**Description:** Label for the communication function with support or the charge point owner. 'Chat' can be used as a verb, noun, or adjective

**Translations:**
- DA: chat
- DE: Chat
- EN: chat
- EN_US: chat
- ES: chat
- FR: chat
- FR_CA: clavardage
- IT: chat
- NL: chatten / chat / chat-
- NO: chat
- RO: chat
- SV: chatt

---

## client secret
**Tag:** industry

**Description:** A secret key known only to the client and server, used to authenticate the client in OAuth or similar authentication protocols, linked to API.

**Translations:**
- DA: klienthemmelighed
- DE: Client-Geheimnis
- EN: client secret
- EN_US: client secret
- ES: secreto del cliente
- FR: secret client
- FR_CA: secret client
- IT: segreto client
- NL: cliëntgeheim
- NO: klienthemmelighet
- RO: secret client
- SV: klienthemlighet

---

## company car
**Tag:** industry

**Description:** A vehicle provided by en employer. In the Monta context, user can obtain a company car agreement, where work-related charges get reimbursed via Monta Wallet financial service.

**Translations:**
- DA: firmabil
- DE: Firmenwagen
- EN: company car
- EN_US: company car
- ES: coche de empresa
- FR: voiture d’entreprise
- FR_CA: voiture d’entreprise
- IT: auto aziendale
- NL: bedrijfsauto
- NO: firmabil
- RO: autovehicul companie
- SV: firmabil

---

## company registration number 
**Tag:** industry

**Description:** A central business ID that identifies a legal entity in official, commercial, and legal contexts within the EU

**Translations:**
- DA: CVR-nummer
- DE: Handelsregisternummer
- EN: company registration number 
- EN_US: employer identification number
- ES: número de identificación fiscal
- FR: numéro SIREN
- FR_CA: numéro d’entreprise du registre (UE)
- IT: numero di iscrizione al registro delle imprese
- NL: KVK-nummer
- NO: organisasjonsnummer
- RO: cod unic de  înregistrare
- SV: organisationsnummer

---

## connect
**Tag:** product

**Description:** Verb. Used when EV is being connected to Monta software or plug-in with a charge point

**Translations:**
- DA: tilslutte
- DE: verbinden
- EN: connect
- EN_US: connect
- ES: conectar
- FR: connecter
- FR_CA: connecter
- IT: connettere
- NL: verbinden
- NO: koble til
- RO: conectare
- SV: ansluta

---

## connector
**Tag:** industry

**Description:** Connectors at each cable end compatible with the EV port and power outlet, e.g. CHAdeMO, Type 2, CCS, Type 1, etc.

**Translations:**
- DA: ladestik
- DE: Anschluss
- EN: connector
- EN_US: connector
- ES: conector
- FR: connecteur
- FR_CA: connecteur
- IT: connettore
- NL: connector
- NO: kontakt
- RO: conector
- SV: laddkontakt

---

## cost group
**Tag:** product

**Description:** A price group type that defines the real cost for the team of a given charge. They represent what the team actually pays for the electricity or charging service, as opposed to what they charge to users. This functionality is a data tracking tool, not a payment system, used to monitor the actual electricity cost of operating charge point.

**Translations:**
- DA: omkostningsgruppe
- DE: Kostengruppe
- EN: cost group
- EN_US: cost group
- ES: grupo de costes
- FR: groupe de coût
- FR_CA: groupe de coût
- IT: gruppo di costo
- NL: kostengroep
- NO: kostnadsgruppe
- RO: grup de costuri
- SV: kostnadsgrupp

---

## credit
**Tag:** product

**Description:** Non-withdrawable funds added to a Team Wallet, used for Monta transactions (charges, fees, purchases). Teams earn credits through referrals, promo codes, or PowerBank

**Translations:**
- DA: kredit
- DE: Guthaben
- EN: credit
- EN_US: credit
- ES: crédito
- FR: crédit
- FR_CA: crédit
- IT: credito
- NL: krediet
- NO: kreditt
- RO: credit
- SV: kredit

---

## credit note
**Tag:** industry

**Description:** A document issued to correct or reduce the amount of a previously issued invoice, often due to returns, discounts, or billing errors. It indicates a credit in favor of the buyer.

**Translations:**
- DA: kreditnota
- DE: Gutschrift
- EN: credit note
- EN_US: credit note
- ES: nota de crédito	
- FR: avoir
- FR_CA: note de crédit
- IT: nota di credito
- NL: creditnota
- NO: kreditnota
- RO: notă de credit
- SV: kreditfaktura

---

## current
**Tag:** industry

**Description:** The flow of electric charge used to power or charge an electric vehicle, typically delivered as either alternating current (AC) or direct current (DC), depending on the charging infrastructure.

**Translations:**
- DA: strøm
- DE: Strom
- EN: current
- EN_US: current
- ES: corriente
- FR: courant
- FR_CA: courant
- IT: corrente
- NL: stroom
- NO: strøm
- RO: curent
- SV: ström

---

## custom subscription
**Tag:** product

**Description:** Feature. A monthly subscription plan that charge point operators offer to their clients, including services such as operation, maintenance, leasing, and repair of charge points. End-users pay a monthly fee via Monta to the operator or service provider

**Translations:**
- DA: brugerdefineret abonnement
- DE: Benutzerdefiniertes Abonnement
- EN: custom subscription
- EN_US: custom subscription
- ES: suscripción personalizada
- FR: abonnement personnalisé
- FR_CA: abonnement personnalisé
- IT: abbonamento personalizzato
- NL: aangepast abonnement
- NO: brukerdefinert abonnement
- RO: abonament personalizat
- SV: specialabonnemang

---

## customer success manager
**Tag:** brand

**Description:** A dedicated support agent, ensuring new users are properly onboarded and succeeding with the Monta software.

**Translations:**
- DA: customer success manager
- DE: Customer Success Manager
- EN: customer success manager
- EN_US: customer success manager
- ES: director de satisfacción del cliente
- FR: responsable de réussite client
- FR_CA: gestionnaire de la réussite client
- IT: responsabile di customer success
- NL: customer success manager
- NO: customer success manager
- RO: customer success manager
- SV: kundhanteringsansvarig

---

## deep link
**Tag:** industry

**Description:** Button label in Hub, URL that navigates directly to specific content within the web-app.

**Translations:**
- DA: deep link
- DE: Deep Link
- EN: deep link
- EN_US: deep link
- ES: hiperenlace
- FR: deep link
- FR_CA: lien direct
- IT: link
- NL: deeplink
- NO: dyplenke
- RO: link direct
- SV: deep link

---

## delayed start
**Tag:** product

**Description:** Scheduling option when using SmartCharge. It is subtitle to a switch that enabled the user to delay the start of the charge.

**Translations:**
- DA: udskudt start
- DE: Verzögerter Start
- EN: delayed start
- EN_US: delayed start
- ES: inicio retrasado
- FR: démarrage différé
- FR_CA: démarrage différé
- IT: partenza ritardata
- NL: uitgestelde start
- NO: forsinket start
- RO: începere întârziată
- SV: fördröjd start

---

## deposit
**Tag:** product

**Description:** Noun, added funds into user's wallet balance and made disposible for payments and transactions. 


**Translations:**
- DA: indbetaling
- DE: Einzahlung
- EN: deposit
- EN_US: deposit
- ES: depósito
- FR: dépôt
- FR_CA: dépôt
- IT: deposito
- NL: storting
- NO: depositum
- RO: depunere
- SV: insättning

---

## distribution system operator (DSO)
**Tag:** industry

**Description:** A DSO (Distribution System Operator) manages and maintains the electricity distribution network that delivers power to end-users. It differs from a utility provider, which generates and supplies electricity

**Translations:**
- DA: distributionssystemoperatør (DSO)
- DE: Verteilnetzbetreiber
- EN: distribution system operator (DSO)
- EN_US: electricity provider
- ES: operador del sistema de distribución (DSO)
- FR: gestionnaire de réseau de distribution
- FR_CA: gestionnaire de réseau de distribution
- IT: operatore del Sistema di Distribuzione (DSO)
- NL: netbeheerder
- NO: nettleverandør
- RO: operator de distribuție (OD)
- SV: systemansvarig för distributionssystem (DSO)

---

## dynamic load balancing
**Tag:** industry

**Description:** Refers to the process of distributing energy across a network of outlets a dynamic and efficient manner, i.e. continuously monitoring the workload on different resources and dynamically adjusting the distribution of tasks or data to ensure optimal utilization and performance.

**Translations:**
- DA: dynamisk load balancing
- DE: Dynamisches Lastmanagement
- EN: dynamic load balancing
- EN_US: dynamic load management
- ES: balanceo de carga dinámico
- FR: répartition de charge dynamique
- FR_CA: répartition de charge dynamique
- IT: bilanciamento del carico dinamico
- NL: dynamic load balancing
- NO: dynamisk lastbalansering
- RO: echilibrarea dinamică a sarcinii
- SV: dynamisk lastbalansering

---

## dynamic price
**Tag:** product

**Description:** A price group with a master price based on the spot price. Dynamic pricing reflects market fluctuations, unlike fixed pricing

**Translations:**
- DA: dynamisk pris
- DE: Dynamischer Preis
- EN: dynamic price
- EN_US: dynamic price
- ES: precio dinámico
- FR: prix dynamique
- FR_CA: tarification dynamique
- IT: prezzo dinamico
- NL: dynamische prijs
- NO: dynamisk pris
- RO: preț dinamic
- SV: dynamiskt pris

---

## e-mobility service provider
**Tag:** industry

**Description:** An e-mobility service provider (eMSP) is a company that offers a range of services related to electric mobility and offer various solutions and infrastructure to facilitate EV charging and mobility services.

**Translations:**
- DA: serviceudbyder af e-mobilitet
- DE: E-mobilitätsdienstleister
- EN: e-mobility service provider
- EN_US: e-mobility service provider
- ES: proveedor de servicios de movilidad eléctrica
- FR: fournisseur de services de mobilité électrique
- FR_CA: fournisseur de services de mobilité électrique
- IT: fornitore di servizi di mobilità elettrica
- NL: aanbieder van e-mobiliteitsdiensten
- NO: leverandør av elbiltjenester
- RO: furnizor de servicii E-mobility
- SV: laddtjänsteleverantör

---

## electricity costs
**Tag:** industry

**Description:** General term, indicating electricity costs. Applied for pricing setting.

**Translations:**
- DA: elomkostninger
- DE: Stromkosten
- EN: electricity costs
- EN_US: electricity costs
- ES: costes de la electricidad
- FR: coûts d’électricité
- FR_CA: coûts d’électricité
- IT: costi dell'energia elettrica
- NL: stroomkosten
- NO: strømkostnader
- RO: costurile energiei electrice
- SV: elkostnader

---

## electricity price
**Tag:** industry

**Description:** Price for electricity set by utility sector.

**Translations:**
- DA: elpris
- DE: Strompreis
- EN: electricity price
- EN_US: electricity price
- ES: precio de la electricidad
- FR: prix de l’électricité
- FR_CA: prix de l’électricité
- IT: prezzo dell'elettricità
- NL: stroomprijs
- NO: strømpris
- RO: prețul energiei electrice
- SV: elpris

---

## electricity tax refund 
**Tag:** product

**Description:** A Danish government scheme that reimburses private EV owners for the electricity tax paid on home charging, provided charging is metered and handled via an approved subscription or provider.

**Translations:**
- DA: elafgiftsrefusion
- DE: Rückerstattung der Stromsteuer
- EN: electricity tax refund 
- EN_US: electricity tax refund 
- ES: reembolso del impuesto eléctrico
- FR: remboursement de la taxe sur l’électricité
- FR_CA: remboursement de la taxe sur l’électricité
- IT: rimborso della tassa sull’elettricità
- NL: terugbetaling van de elektriciteitsbelasting
- NO: refusjon av elavgift	
- RO: rambursarea taxei pe electricitate
- SV: återbetalning av elskatt

---

## electricity tax refund subscription
**Tag:** brand

**Description:** Service/plan type enabling tax refund when renting out private charge point to a 3rd party. Relevant only in Denmark.

**Translations:**
- DA: elrefusionsabonnement
- DE: Stromsteuerrückerstattungsabo
- EN: electricity tax refund subscription
- EN_US: electricity tax refund subscription
- ES: suscripción al reembolso del impuesto eléctrico
- FR: abonnement pour remboursement de la taxe sur l’électricité
- FR_CA: abonnement pour remboursement de la taxe sur l’électricité
- IT: abbonamento per il rimborso della tassa sull’elettricità
- NL: abonnement voor terugbetaling van de elektriciteitsbelasting
- NO: abonnement på refusjon av elavgift
- RO: abonament pentru rambursarea taxei pe electricitate
- SV: abonnemang för återbetalning av elskatt

---

## end time
**Tag:** product

**Description:** Functionality in Scheduling feature, identify when charge window should end, e.g. charge to kick in in the timespan between 1.00 a.m - 6 a.m.

**Translations:**
- DA: sluttidspunkt
- DE: Endzeit
- EN: end time
- EN_US: end time
- ES: hora de finalización
- FR: heure de fin
- FR_CA: heure de fin
- IT: ora di fine
- NL: eindtijd
- NO: sluttid
- RO: ora de încheiere
- SV: sluttid

---

## excise tax
**Tag:** industry

**Description:** Geo-specific tax imposed during EV charging in the US. No direct tax directive is applicable in EU. For translation purposes, use similar taxation concept for the local language. 


**Translations:**
- DA: elafgift 
- DE: Stromsteuer
- EN: excise tax 
- EN_US: excise tax
- ES: impuesto especial
- FR: accise
- FR_CA: taxe d’accise
- IT: accisa
- NL: energiebelasting
- NO: elavgift
- RO: acciză
- SV: energiskatt

---

## fail-safe
**Tag:** industry

**Description:** A safety measure that reduces the amperage on all chargers if the connection to a meter is lost, until the connection is re-established. Synonym: fallback.

**Translations:**
- DA: fallback
- DE: Ausfallsicher
- EN: fail-safe
- EN_US: fail-safe
- ES: a prueba de fallos
- FR: sécurité intégrée
- FR_CA: sécurité intégrée
- IT: sicurezza integrata
- NL: uitvalbeveiliging
- NO: sikkerhetsmodus
- RO: siguranță la defecțiune
- SV: säkerhetsfallback 

---

## fair price
**Tag:** product

**Description:** Concept to benchmark good/bad market price on a public charge point for end-users. A filtering option in Maps.

**Translations:**
- DA: fair pris
- DE: Fairer Preis
- EN: fair price
- EN_US: fair price
- ES: precio justo/precio adecuado
- FR: prix équitable
- FR_CA: prix équitable
- IT: prezzo equo
- NL: eerlijke prijs
- NO: rettferdig pris
- RO: preț rezonabil
- SV: rättvist pris

---

## fallback price
**Tag:** industry

**Description:** Price type, Business partners can set up in cases, when Monta cannot follow the spot price, i.e. the fallback price will be enabled in those cases.

**Translations:**
- DA: standardpris
- DE: Standardpreis
- EN: fallback price
- EN_US: fallback price
- ES: precio de reserva
- FR: prix alternatif
- FR_CA: tarif de repli
- IT: prezzo alternativo
- NL: standaardprijs
- NO: standardpris
- RO: tarife alternative
- SV: reservpris

---

## fee
**Tag:** product

**Description:** Used widely and describes different fees, e.g. member fee, additional fees. This allow pricing model to be flexible

**Translations:**
- DA: gebyr
- DE: Gebühr
- EN: fee
- EN_US: fee
- ES: tasa
- FR: frais
- FR_CA: frais
- IT: costo
- NL: kosten
- NO: avgift
- RO: Taxă
- SV: avgift

---

## fee per minute
**Tag:** product

**Description:** Type of fee (visible to users) applied after a specified charging time, continuing until the cable is disconnected. A fee per minute, e.g., DKK 1.00/min, is added after the first 60 minutes of charging

**Translations:**
- DA: minutgebyr
- DE: Minutengebühr
- EN: fee per minute
- EN_US: fee per minute
- ES: tasa por minuto
- FR: frais à la minute
- FR_CA: frais à la minute
- IT: tariffa al minuto
- NL: kosten per minuut
- NO: minuttpris
- RO: taxă/minut
- SV: minutavgift

---

## firmware
**Tag:** industry

**Description:** Software that provides basic machine instructions enabling hardware to function and communicate with other software on a device

**Translations:**
- DA: firmware
- DE: Firmware
- EN: firmware
- EN_US: firmware
- ES: firmware
- FR: micrologiciel
- FR_CA: micrologiciel
- IT: firmware
- NL: firmware
- NO: fastvare
- RO: firmware
- SV: programvara

---

## fixed price
**Tag:** product

**Description:** Type of pricing scheme with fixed price, visible to end-users. Fixed price in contrary to e.g spot/dynamic price.

**Translations:**
- DA: fast pris
- DE: Festpreis
- EN: fixed price
- EN_US: fixed price
- ES: precio fijo
- FR: prix fixe
- FR_CA: prix fixe
- IT: prezzo fisso
- NL: vaste prijs
- NO: fastpris
- RO: preț fix
- SV: fast pris

---

## fleet
**Tag:** industry

**Description:** A collection of vehicles owned or operated by an entity. Fleet management covers acquisition, maintenance, utilization, and disposal of these vehicles

**Translations:**
- DA: flåde
- DE: Flotte
- EN: fleet
- EN_US: fleet
- ES: flota
- FR: flotte
- FR_CA: flotte
- IT: flotta
- NL: wagenpark
- NO: flåte
- RO: flotă
- SV: vagnpark

---

## forecast
**Tag:** product

**Description:** Refers to forecasted data points such as CO2 levels, renewable energy, and SmartCharge forecasts. Displayed in the consumer app under Insights in the private charge point settings.

**Translations:**
- DA: prognoser
- DE: Prognose
- EN: forecast
- EN_US: forecast
- ES: previsiones
- FR: prévisions
- FR_CA: prévisions
- IT: previsioni
- NL: voorspelling
- NO: prognoser
- RO: prognoze
- SV: prognoser

---

## funds
**Tag:** product

**Description:** Withdrawable funds that can be transferred to a user's private bank account. The Wallet offers a withdrawal feature, allowing funds to be withdrawn on demand or set for automatic withdrawal.

**Translations:**
- DA: penge, midler beløb
- DE: Auszahlbares Guthaben
- EN: fund
- EN_US: fund
- ES: fondos
- FR: fonds
- FR_CA: fonds
- IT: fondo
- NL: saldo
- NO: midler
- RO: fonduri
- SV: saldo

---

## fuse protection
**Tag:** brand

**Description:** B2Cfeature in mobile app, prevents main fuse overload during EV charging by applying a static limit or using smart meter data for dynamic control. Charging is reduced or paused as needed to avoid outages. Supports single or multiple charge points.

**Translations:**
- DA: sikringsbeskyttelse
- DE: Sicherungsschutz
- EN: fuse protection
- EN_US: fuse protection
- ES: protección de fusibles
- FR: protection du fusible
- FR_CA: protection par fusible
- IT: protezione del fusibile
- NL: zekeringbescherming
- NO: sikringsvern
- RO: protecția siguranței
- SV: säkringsskydd

---

## grid
**Tag:** industry

**Description:** Refers to the interconnected network of power generation, transmission lines, substations, and distribution systems that deliver electricity to consumers.

**Translations:**
- DA: elnet
- DE: Stromnetz
- EN: grid
- EN_US: grid
- ES: red eléctrica/red
- FR: réseau
- FR_CA: réseau
- IT: rete elettrica
- NL: stroomnet
- NO: strømnett
- RO: rețea
- SV: nät

---

## hardware
**Tag:** industry

**Description:** A charging device, i.e. charge point.

**Translations:**
- DA: hardware
- DE: Hardware
- EN: hardware
- EN_US: hardware
- ES: hardware
- FR: équipement
- FR_CA: équipement
- IT: hardware
- NL: hardware
- NO: maskinvare
- RO: hardware
- SV: maskinvara

---

## hardware agnostic
**Tag:** industry

**Description:** An adjective describing Monta's software, compatible with various charging hardware brands, enabling EV owners to charge at different stations regardless of the hardware brand.

**Translations:**
- DA: hardwareagnostisk
- DE: herstellerunabhängig
- EN: hardware agnostic
- EN_US: hardware agnostic
- ES: independiente de hardware
- FR: indépendant du matériel
- FR_CA: indépendant de l’équipement
- IT: indipendente dall'hardware
- NL: hardwareonafhankelijk
- NO: maskinvareuavhengig OR maskinvareagnostisk
- RO: compatibil cu majoritatea dispozitivelor
- SV: maskinvaruoberoende

---

## hardware manufacturer
**Tag:** industry

**Description:** A producer of charge point hardware

**Translations:**
- DA: hardwareproducent
- DE: B2B: Hardware-Hersteller; B2C: Hersteller der Ladestation
- EN: hardware manufacturer
- EN_US: hardware manufacturer
- ES: fabricante de cargadores
- FR: constructeur
- FR_CA: fabricant d’équipement
- IT: produttore hardware
- NL: hardwarefabrikant
- NO: ladestasjonprodusent
- RO: producătorul stației de încărcare
- SV: hårdvaruleverantör

---

## home charger
**Tag:** industry

**Description:** Refers to charge point at home owned by a private-user

**Translations:**
- DA: hjemmelader
- DE: Heimladestation
- EN: home charger
- EN_US: home charger
- ES: cargador doméstico
- FR: borne domestique
- FR_CA: borne résidentielle
- IT: punto di ricarica domestico
- NL: thuislaadpaal
- NO: hjemmelader
- RO: încărcător personal
- SV: hemmaladdare

---

## home charging
**Tag:** industry

**Description:** A service enabled by the Charge app, allowing home users to manage charging, share charge points within their network, or make them publicly available

**Translations:**
- DA: hjemmeladning
- DE: Zuhause laden
- EN: home charging
- EN_US: home charging
- ES: carga en casa/carga doméstica
- FR: recharge à domicile
- FR_CA: recharge à domicile
- IT: ricarica domestica
- NL: thuisladen
- NO: hjemmelading
- RO: încărcare personală

- SV: laddning hemma

---

## home energy
**Tag:** brand

**Description:** A residential EV charging solution in the Charge app, that optimizes charging costs with features like smart charging, solar charging. It connects users to their home charge points, using weather forecasts, solar data, and smart meters for efficient eletricty consumption.

**Translations:**
- DA: hjemmeenergi
- DE: Heimenergie
- EN: home energy
- EN_US: home energy
- ES: energía en casa
- FR: énergie à domicile
- FR_CA: énergie à domicile
- IT: energia domestica
- NL: energie thuis
- NO: hjemmeenergi
- RO: energie acasă
- SV: hemenergi

---

## housing association
**Tag:** industry

**Description:** In this context, a housing association refers to an entity with a shared charge point installed for use by its residents. Opt for local terms, that resonate with the concept.  

**Translations:**
- DA: boligforening
- DE: Wohnungsgesellschaft (WEG)
- EN: housing association
- EN_US: multi-family residence, residential Site
- ES: complejo residencial
- FR: copropriété
- FR_CA: copropriété
- IT: comproprietà
- NL: woningcorporatie / woningbouwvereniging
- NO: borettslag
- RO: asociație de proprietari
- SV: bostadsrättsförening

---

## idle fee
**Tag:** industry

**Description:** Fee type visible to end-users. An additional fee is applied after the scheduled charge is complete and if a car is not removed from the charging station. An incentive to optimize charge point availability.

**Translations:**
- DA: blokeringsgebyr
- DE: Blockiergebühr
- EN: idle fee
- EN_US: idle fee
- ES: tasa por inactividad
- FR: frais d’inactivité
- FR_CA: frais d’inactivité
- IT: tariffa per inattività
- NL: blokkeertarief
- NO: sperreavgift
- RO: taxă blocare stație
- SV: blockeringsavgift

---

## idle time
**Tag:** industry

**Description:** The time period when a charge point is occupied but not charging, known as the blocking period, during which an idle fee is applied.

**Translations:**
- DA: blokeringsperiode
- DE: Blockierzeit
- EN: idle time
- EN_US: idle time
- ES: tiempo de inactividad
- FR: temps d’inactivité
- FR_CA: temps d’inactivité
- IT: tempo di inattività
- NL: blokkeertijd
- NO: sperreavgift
- RO: timp blocare
- SV: blockeringstid

---

## incident
**Tag:** product

**Description:** Operational event/fault in charging operations (e.g., charge point disconnected, error state). Related alert feature

**Translations:**
- DA: hændelse
- DE: störung
- EN: incident
- EN_US: incident
- ES: incidencia
- FR: incident
- FR_CA: incident
- IT: incidente
- NL: incident
- NO: hendelse
- RO: incident
- SV: händelse

---

## installation
**Description:** A physical installation of charge point, including connecting to electricity and establishing an internet connection (SIM card, LAN cable)  A certified installer set it up. 

**Translations:**
- DA: installation
- DE: Installation
- EN: installation
- EN_US: installation
- ES: instalación
- FR: installation 
- FR_CA: installation 
- IT: installazione
- NL: installatie
- NO: installasjon
- RO: instalarea
- SV: installation

---

## installer
**Tag:** industry

**Description:** An authorized electrician, who sets charge points and runs maintenance.

**Translations:**
- DA: installatør
- DE: Installateur
- EN: installer
- EN_US: installer
- ES: instalador
- FR: installateur
- FR_CA: installateur
- IT: installatore
- NL: installateur
- NO: installatør, montør
- RO: instalator
- SV: installatör

---

## installer job
**Tag:** product

**Description:** Source and target capitalization intact. Feature for operators or a site owners to easily provide access to installers. Smooth method for installers to test and connect to the charge points, without needing Team access.

**Translations:**
- DA: installatørjob
- DE: Installateur-Auftrag
- EN: installer job
- EN_US: installer jobs
- ES: trabajo del instalador
- FR: projet installateur
- FR_CA: tâche d’installation
- IT: interventi installatore
- NL: installateursfunctie
- NO: installeringsjobb
- RO: job instalator
- SV: installationsjobb

---

## integration
**Tag:** industry

**Description:** In Monta's context, it refers to integrating charge points with Monta's software, i.e. pointing charge points to the Monta's backend.  Charge point needs to be installed before integrating it with the software. 

**Translations:**
- DA: integration
- DE: Integration
- EN: integration
- EN_US: integration
- ES: integración
- FR: intégration
- FR_CA: intégration
- IT: integrazione
- NL: integratie
- NO: integrering
- RO: integrare
- SV: integration

---

## invoice
**Tag:** industry

**Description:** Refers to b2b and b2p segments, issues each month, containing transaction overview.

**Translations:**
- DA: faktura
- DE: Rechnung
- EN: invoice
- EN_US: invoice
- ES: factura
- FR: facture
- FR_CA: facture
- IT: fattura
- NL: factuur
- NO: faktura
- RO: factură
- SV: faktura

---

## join code
**Tag:** product

**Description:** A code, that enables a new team member to join a Team. It's being generated by a Team administrator.

**Translations:**
- DA: joinkode
- DE: Beitrittscode
- EN: join code
- EN_US: join code
- ES: código de inscripción
- FR: code d'accès
- FR_CA: code d’adhésion
- IT: codice team
- NL: deelnamecode
- NO: deltakerkode
- RO: cod de înscriere
- SV: anslutningskod

---

## kW effect
**Tag:** industry

**Description:**  Unit of power = kilowatt (kW) defines the amount of power EV battery received in a charge. kW input / power input ( kW)

**Translations:**
- DA: kW-ladeeffekt
- DE: kW-Effekt
- EN: kW effect
- EN_US: kW effect
- ES: potencia de carga
- FR: puissance de charge
- FR_CA: puissance de charge
- IT: potenza di carica
- NL: kW-effect
- NO: kW-ladeeffekt
- RO: putere de încărcare
- SV: kW-effekt

---

## kWh price
**Tag:** product

**Description:** Price variable used when setting up a price group. 

**Translations:**
- DA: kWh-pris
- DE: kWh-Preis
- EN: kWh price
- EN_US: kWh price
- ES: precio del kWh
- FR: prix du kWh
- FR_CA: prix du kWh
- IT: prezzo al kWh
- NL: kWh-prijs
- NO: pris per kWh
- RO: preț kWh
- SV: kWh-pris

---

## load balancing
**Tag:** industry

**Description:** Legacy term, now replaced by "load management" - a broader imdustry term, where load balancing is sub-term. Distribution of available current between multiple charge points to optimize current usage.

**Translations:**
- DA: load balancing
- DE: Lastmanagement
- EN: load balancing
- EN_US: load management
- ES: balanceo de carga
- FR: répartition de charge
- FR_CA: répartition de charge
- IT: bilanciamento del carico
- NL: load balancing
- NO: lastbalansering
- RO: echilibrarea sarcinii
- SV: lastbalansering

---

## load management 
**Tag:** industry

**Description:** Load management is both an industry term and feature name. it is system for distributing power among multiple charge points that are charging simultaneously. It allows installation of more charge points without increasing electrical installation capacity by analyzing available capacity and vehicle demand to ensure capacity limits aren't exceeded.

**Translations:**
- DA: load management
- DE: Lastmanagement
- EN: load management 
- EN_US: load management
- ES: balanceo de carga
- FR: gestion de la charge
- FR_CA: gestion de la charge
- IT: load management
- NL:         laadbeheer
- NO: laststyring
- RO: load management
- SV: lastbalansering

---

## marketplace
**Tag:** product

**Description:** Feature, where Monta partners up-sell EV charging subscriptions and services to drivers and businesses through the Monta App and Hub
Feature name (DNT). Lowercase in all locales. English only. Do not translate or inflect.

**Translations:**
- DA: marketplace
- DE: marketplace
- EN: marketplace
- EN_US: marketplace
- ES: marketplace
- FR: marketplace
- FR_CA: marketplace
- IT: marketplace
- NL: marketplace
- NO: marketplace
- RO: marketplace
- SV: marketplace

---

## master price
**Tag:** product

**Description:** Core price set per kWh, per minute, or as a spot price, forming the basis of a price group with additional fees. Set by a Team administrator and managed in Hub

**Translations:**
- DA: masterpris
- DE: Hauptpreis
- EN: master price
- EN_US: master price
- ES: precio maestro
- FR: prix de base
- FR_CA: prix de base
- IT: prezzo di base
- NL: basisprijs
- NO: hovedpris
- RO: prețul principal
- SV: baspris

---

## member
**Tag:** product

**Description:** Refer's to a Team member

**Translations:**
- DA: medlem
- DE: Mitglied
- EN: member
- EN_US: member
- ES: miembro
- FR: membre
- FR_CA: membre
- IT: membro
- NL: lid
- NO: medlem
- RO: membru
- SV: medlem

---

## member fee
**Tag:** product

**Description:** Refers to a fee applied by a Team admin, automatically transferred from a member's Wallet to the Team's Wallet monthly. Used in contexts like housing associations to collect payments for shared charge points

**Translations:**
- DA: medlemsgebyr
- DE: Mitgliedsbeitrag
- EN: member fee
- EN_US: member fee
- ES: cuota para miembros
- FR: frais d’adhésion
- FR_CA: frais de membres
- IT: quota membro
- NL: lidmaatschapskosten
- NO: medlemsavgift
- RO: taxă de membru
- SV: medlemsavgift

---

## member price
**Tag:** product

**Description:** Member price is applied specifically for members of a particular team.  There are four types: member, public, reimbursed, and cost price group. Each group has a master price (per minute, kWh, or spot price) and additional fees (start, minute, charging, idle).

**Translations:**
- DA: medlemspris
- DE: Mitgliedspreis
- EN: member price
- EN_US: member price
- ES: precio para miembros
- FR: prix pour les membres
- FR_CA: prix pour les membres
- IT: prezzo per i membri
- NL: ledenprijs
- NO: medlemspris
- RO: preț membru
- SV: medlemspris

---

## member price group
**Tag:** product

**Description:** A pricing feature that allows operators to define how much team members pay for charging their EVs on charge points within their team

**Translations:**
- DA: medlemsprissats
- DE: Medlemsprisgrupp
- EN: member price group
- EN_US: member price group
- ES: grupo de precios para miembros
- FR: prix pour les membres
- FR_CA: groupe tarifaire de membre
- IT: config. di prezzo membri
- NL: ledenprijsgroep
- NO: medlemsprisgruppe        
- RO: grup de tarifare pentru membri
- SV: medlemsprisgrupp        

---

## member role
**Tag:** product

**Description:** Functionality related to access and permission roles. A user setting that categorizes team members into three groups: Team Owner, Team Administrator, and Team Member. Each role grants specific rights and access within the team.

**Translations:**
- DA: medlemsrolle
- DE: Mitgliedsrolle
- EN: member role
- EN_US: member role
- ES: rol de miembro
- FR: rôle de membre
- FR_CA: rôle de membre
- IT: ruolo del membro
- NL: ledenrol
- NO: brukerrolle
- RO: rol membru
- SV: medlemsroll

---

## meter ID
**Tag:** industry

**Description:** A unique number assigned to an individual meter to distinguish it from others within a utility's network

**Translations:**
- DA: elmåler-id
- DE: Zähler-ID
- EN: meter ID
- EN_US: electricity meter ID
- ES: ID del contador eléctrico
- FR: ID du compteur électrique
- FR_CA: ID du compteur électrique
- IT: ID contatore elettricità
- NL: stroommeter-ID
- NO: måler-ID
- RO: ID-ul contorului de energie electrică
- SV: anläggnings-ID

---

## meter accuracy
**Tag:** industry

**Description:** Refers to the accuracy of the electric meter in measuring energy consumed during an EV charging session, indicating how closely the readings match actual consumption.

**Translations:**
- DA: målernøjagtighed
- DE: Messgenauigkeit
- EN: meter accuracy
- EN_US: meter accuracy
- ES: precisión de medición
- FR: précision du compteur
- FR_CA: précision du compteur
- IT: precisione del contatore
- NL: meternauwkeurigheid
- NO: målerens nøyaktighet
- RO: precizia contorului
- SV: mätarnoggrannhet

---

## meter reading
**Tag:** product

**Description:** Devise tracking electricity consumption.

**Translations:**
- DA: måleraflæsning
- DE: Zählermesser
- EN: meter reading
- EN_US: meter reading
- ES: lectura de medidor
- FR: relevés du compteur
- FR_CA: lecture du compteur
- IT: lettura contatore
- NL: stroommeter
- NO: måleravlesning
- RO: citire contor
- SV: mätaravläsning

---

## multi-level load balancing
**Tag:** industry

**Description:** Ability to load balance between a main fuse and lower-level groups.

**Translations:**
- DA: multi-level load balancing
- DE: Mehrstufiges Lastmanagement
- EN: multi-level load balancing
- EN_US: multi-level load management
- ES: balanceo de carga entre varios niveles
- FR: répartition de charge multi-niveaux
- FR_CA: gestion de charge à plusieurs niveaux
- IT: bilanciamento del carico multilivello
- NL: multi-level load balancing
- NO: multi-level lastbalansering
- RO: echilibrarea sarcinii pe niveluri multiple
- SV: lastbalansering på flera nivåer

---

## native schedule
**Tag:** product

**Description:** Describes a phase in the charging session, where EV receives a small amount of current before it becomes full power. This way improves charge session stability, reduces failed charges.

**Translations:**
- DA: Native schedule
- DE: Native Schedule
- EN: native schedule
- EN_US: native schedule
- ES: programación nativa
- FR: programmation native
- FR_CA: programmation native
- IT: programmazione nativa
- NL: native Schedule
- NO: native schedule
- RO: sarcină electrică prealabilă
- SV: grundschema

---

## nested team
**Tag:** product

**Description:** Functionality in Hub allowing the invitation of users to a parent team (host) and granting access to charge points with the set pricing scheme. Target users include utility companies, enterprises, and housing associations.

**Translations:**
- DA: indlejret team
- DE: Verschachteltes Team
- EN: nested team
- EN_US: nested team
- ES: equipo anidado
- FR: groupe imbriqué
- FR_CA: équipe imbriquée
- IT: team integrato
- NL: gastteam
- NO: innebygd team
- RO: grup integrat
- SV: underordnat team

---

## network overview dashboard 
**Tag:** product

**Description:** B2B feature. Real-time cockpit in Hub showing fleet health and alerts so teams can spot issues, prioritize, and resolve proactively

**Translations:**
- DA: netværksoversigt dashboard
- DE: Netzwerkübersicht Dashboard
- EN: network overview dashboard 
- EN_US: network overview dashboard 
- ES: panel de control – operaciones de red
- FR: tableau de bord – opérations réseau
- FR_CA: tableau de bord – opérations réseau
- IT: dashboard operazioni rete
- NL: netwerkoverzicht dashboard
- NO: nettverksoversikt dashboard
- RO: dashboard operațiuni rețea
- SV: nätverksöversikt dashboard

---

## network tariff
**Tag:** industry

**Description:** A fee charged by utility companies for using their energy distribution networks.

**Translations:**
- DA: nettarif
- DE: Netzentgelt
- EN: network tariff
- EN_US: delivery fee
- ES: tarifa de red
- FR: tarif d’utilisation des réseaux
- FR_CA: sur place
- IT: tariffa di utilizzo della rete
- NL: netwerktarief
- NO: nettverkstariff
- RO: tarif de rețea
- SV: elnätsavgift

---

## on-site
**Tag:** industry

**Description:** Used as adjective and noun. E.g. on-site maintenance, on-site charging, solve issues on-site.

**Translations:**
- DA: på stedet
- DE: am Standort
- EN: on-site
- EN_US: on-site
- ES: in situ
- FR: sur site
- FR_CA: sur site
- IT: sul posto
- NL: op locatie
- NO: på stedet
- RO: la fața locului
- SV: på plats

---

## operator 
**Tag:** industry

**Description:** Businesses selling, installing, and setting up charge points for themselves or their clients.

**Translations:**
- DA: operatør
- DE: Betreiber
- EN: operator
- EN_US: operator (UI) / turnkey provider (marketing)
- ES: operador
- FR: opérateur
- FR_CA: opérateur
- IT: operatore
- NL: exploitant
- NO: operatør
- RO: operator
- SV: operatör

---

## operator team
**Tag:** product

**Description:** Team type for partners. An operator team is equal to a professional team, with the only difference that any income on the team wallet on an operator team, will automatically be sent from the team wallet directly to the operator wallet. The Operator will then use this charge income to cover the invoice to Monta.

**Translations:**
- DA: operatørteam
- DE: Betreiberteam
- EN: operator team
- EN_US: operator team
- ES: equipo operador
- FR: groupe de l’opérateur
- FR_CA: équipe d’opérateurs
- IT: team operatore
- NL: operator team
- NO: operatørteam
- RO: grup de operatori
- SV: operatörsteam

---

## pair
**Tag:** product

**Description:**  Verb, to link or connect a charge key (such as an RFID card) with a charge point or user account in a platform, allowing the key to be recognized and authorized for charging sessions.

**Translations:**
- DA: tilføj
- DE: koppeln
- EN: pair
- EN_US: pair
- ES: emparejar
- FR: associer
- FR_CA: associer
- IT: connetti
- NL: koppelen
- NO: koble
- RO: asocia
- SV: anslut

---

## parent team
**Tag:** product

**Description:** Functionality in Hub. Tied to Nested Teams. It is a hosting Team of a Nested Team. Source and target capitalization intact.

**Translations:**
- DA: overordnet team
- DE: Übergeordnetes Team
- EN: parent team
- EN_US: parent team
- ES: equipo principal
- FR: groupe principal
- FR_CA: équipe principale
- IT: team principali
- NL: hoofdteam
- NO: overordnet team
- RO: grup principal
- SV: överordnat team

---

## passcode
**Tag:** industry

**Description:** 4-digit authentication code required upon handling payments via Monta Wallet. EU + UK requirement in the payment law.

**Translations:**
- DA: pinkode
- DE: Zugangscode
- EN: passcode
- EN_US: passcode
- ES: código de acceso
- FR: code d’accès
- FR_CA: code d’accès
- IT: PIN
- NL: pincode
- NO: pinkode
- RO: cod de acces
- SV: pinkod

---

## password
**Tag:** industry

**Description:** A standard term for a password used to log into Monta systems.

**Translations:**
- DA: adgangskode
- DE: Passwort
- EN: password
- EN_US: password
- ES: contraseña
- FR: mot de passe
- FR_CA: mot de passe
- IT: password
- NL: wachtwoord
- NO: passord
- RO: parolă
- SV: lösenord

---

## payment
**Tag:** product

**Description:** Label of 'Payment' button in Hub. It provides an overview of Wallet balance, transactions & invoices, as well as enables deposit, and withdrawal of funds.

**Translations:**
- DA: betaling
- DE: Zahlung
- EN: payment
- EN_US: payment
- ES: pago
- FR: paiement
- FR_CA: paiement
- IT: pagamento
- NL: betaling
- NO: betaling
- RO: Plată
- SV: betalning

---

## payment method
**Tag:** product

**Description:** Label of subheader in the Wallet, i.e. credit card, RFID, MobilePay, Vipps.

**Translations:**
- DA: betalingsmetode
- DE: Zahlungsmethode
- EN: payment method
- EN_US: payment method
- ES: método de pago
- FR: moyen de paiement
- FR_CA: mode de paiement
- IT: metodo di pagamento
- NL: betalingsmethode
- NO: betalingsmetode
- RO: metoda de plată
- SV: betalsätt

---

## payment terminal
**Tag:** product

**Description:** A payment terminal is a device used to process electronic payments from customers.

**Translations:**
- DA: betalingsterminal
- DE: Zahlungsterminal
- EN: payment terminal
- EN_US: payment terminal
- ES: terminal de pago
- FR: terminal de paiement
- FR_CA: terminal de paiement
- IT: terminale di pagamento
- NL: betalingsterminal
- NO: betalingsterminal
- RO: terminal de plată
- SV: betalterminal

---

## peak hours
**Tag:** industry

**Description:** The timeframe, in which many users want to charge.

**Translations:**
- DA: spidsbelastningsperiode
- DE: Stoßzeiten
- EN: peak hours
- EN_US: peak hours
- ES: horas pico
- FR: heures de pointe
- FR_CA: heures de pointe
- IT: ore di punta
- NL: piekuren
- NO: rushtid
- RO: ore de vârf
- SV: högtrafiktimmar

---

## personal tax number
**Tag:** industry

**Description:** Established taxation term applied in b2c context.  Unique identifier assigned to individuals by a government’s tax authority, used for managing tax-related activities. 

**Translations:**
- DA: CPR-nummer (CPR)
- DE: Steuer-ID
- EN: personal tax number
- EN_US:  social security number (SSN)
- ES: número de identificación fiscal (NIF)
- FR: numéro fiscal (NIF)
- FR_CA: numéro d’assurance sociale (NAS)
- IT: codice fiscale
- NL: burgerservicenummer (BSN)
- NO: skattenummer (TIN)
- RO: cod numeric personal (CNP)
- SV: personnummer

---

## price group
**Tag:** product

**Description:** A price configuration setting enabling to set-up end-user price, containing marging, add ons. etc.

**Translations:**
- DA: prisgruppe
- DE: Preisgruppe
- EN: price group
- EN_US: price group
- ES: grupo de precios
- FR: prix de groupe
- FR_CA: groupe tarifaire
- IT: configura gruppo prezzi
- NL: prijsgroep
- NO: prisgruppe
- RO: grup tarifar
- SV: prisgrupp

---

## private charge point
**Tag:** product

**Description:** Charge point access rights: Only team members linked to a private charge point can access it and apply pricing. Public charge points are accessible via roaming

**Translations:**
- DA: privat ladestander
- DE: private Ladestation
- EN: private charge point
- EN_US: private charger
- ES: punto de recarga privado
- FR: borne de recharge privée
- FR_CA: bornes privée
 bornes de recharge privée
- IT: punto di ricarica privato
- NL: privélaadpaal
- NO: privat ladepunkt
- RO: stație de încărcare privată
- SV: privat laddpunkt

---

## private team
**Tag:** product

**Description:** Team type for private home users and partners, created in Hub, allowing the owner to invite multiple home users

**Translations:**
- DA: privat team
- DE: privates Team
- EN: private team
- EN_US: private team
- ES: equipo privad
- FR: groupe privé
- FR_CA: équipe privée
- IT: team privato
- NL: privéteam
- NO: privat team
- RO: grup privat
- SV: privat team

---

## production forecast
**Tag:** product

**Description:** A prediction of how much electricity a solar panel system will generate over a specific period, based on system specifications and local weather conditions

**Translations:**
- DA: Produktionsprognose
- DE: ertragsprognose
- EN: production forecast
- EN_US: production forecast
- ES: pronóstico de producción
- FR: prévision de production
- FR_CA: prévision de production
- IT: previsione di produzione
- NL: productievoorspelling
- NO: produksjonsprognose
- RO: previziune de producție
- SV: produktionsprognos

---

## professional team
**Tag:** product

**Description:** Team type for housing associations and businesses

**Translations:**
- DA: professionelt team
- DE: professionelles Team
- EN: professional team
- EN_US: professional team
- ES: equipo profesional
- FR: groupe professionnel
- FR_CA: équipe professionnelle
- IT: team professionale
- NL: professioneel team
- NO: profesjonelt team
- RO: grup profesional
- SV: professionellt team

---

## promotion code
**Tag:** product

**Description:** A code that gives the user a discount on a charge. This will be stored in user's Wallet.

**Translations:**
- DA: kampagnekode
- DE: Aktionscode
- EN: promotion code
- EN_US: promotion code
- ES: código promocional
- FR: code promotionnel
- FR_CA: code promotionnel
- IT: codice promozionale
- NL: kortingscode
- NO: promokode
- RO: cod promoțional
- SV: kampanjkod

---

## public charge point
**Tag:** industry

**Description:** Publicly available charge point.

**Translations:**
- DA: offentlig ladestander
- DE: Öffentliche Ladestation
- EN: public charge point
- EN_US: public charger
- ES: punto de recarga público
- FR: borne de recharge publique
- FR_CA: borne publique
 borne de recharge publique
- IT: punto di ricarica pubblico
- NL: openbare laadpaal
- NO: offentlig ladepunkt
- RO: stație de încărcare publică
- SV: offentlig laddpunkt

---

## public price
**Tag:** product

**Description:** Public price is applied to publicly available charge points. There are 4 price groups in total, i.e. member, public, reimbursed, and cost price group.  A price group is comprised of a master price (set per minute, kWh or spot price) and additional fees ( minute, stat, charging, idle fee)

**Translations:**
- DA: offentlig pris
- DE: Öffentlicher Preis
- EN: public price
- EN_US: public price
- ES: precio público
- FR: prix public
- FR_CA: prix public
- IT: prezzo pubblico
- NL: openbare prijs
- NO: offentlig pris
- RO: preț public
- SV: offentligt pris

---

## reboot
**Tag:** product

**Description:** Verb, to shut down and restoring power to a charge point, either manually or remotely, to restart it. Can be used as verb and noun.

**Translations:**
- DA: genstarte
- DE: neu starten
- EN: reboot
- EN_US: reboot
- ES: reiniciar
- FR: redémarrer
- FR_CA: redémarrer
- IT: riavviare
- NL: herstarten
- NO: start på nytt
- RO: reporni
- SV: starta om

---

## recipient code 
**Tag:** industry

**Description:** Italian billing compliance term, used in Italy-only to identify the recipient of an e-Invoice). No equlavent concepts in other languages, thus do a literal translation. 

**Translations:**
- DA: modtagerkode
- DE: Empfängercode
- EN: recipient code 
- EN_US: recipient code 
- ES: código del destinatario
- FR: code destinataire
- FR_CA: code destinataire
- IT: codice destinatario
- NL: ontvangstcode
- NO: mottakerkode
- RO: cod destinatar
- SV: mottagarkod

---

## referral
**Tag:** product

**Description:** Used in the context of Referal feature, where an existing user can invite a new member and receive a charging balance of certain amount

**Translations:**
- DA: henvisning
- DE: Empfehlung
- EN: referral
- EN_US: referral
- ES: referencia
- FR: parrainage
- FR_CA: référence
- IT: invito
- NL: verwijzing
- NO: henvisning
- RO: recomandare
- SV: värvning

---

## refill
**Tag:** product

**Description:** Verb. Top off a Wallet. Sometimes used also "charge", make sure to distinguish between monetary and electricity charge.

**Translations:**
- DA: tanke op
- DE: auffüllen
- EN: refill
- EN_US: refill
- ES: recargar / rellenar
- FR: recharger
- FR_CA: ajouter des fonds
- IT: ricaricare
- NL: aanvullen
- NO: påfyll, fylle på
- RO: realimentare
- SV: fylla på

---

## reimbursement
**Tag:** product

**Description:** A feature, allowing employers to reimburse their employees for charge sessions. Variations Home Reimbursement ( b2c, Charge mobile app) Employer Reimbursement (b2b, Hub web app)  Replacement of the legacy feature name "Sponsored Charging".

**Translations:**
- DA: firmabetalt opladning
- DE: Rückerstattungen
- EN: reimbursement
- EN_US: reimbursement
- ES: reembolso
- FR: remboursement
- FR_CA: remboursement
- IT: rimborso
- NL: vergoeding
- NO: hjemmeladningsrefusjon
- RO: rambursare încărcare acasă
- SV: återbetalning

---

## reimbursement price
**Tag:** product

**Description:** A money amount per kWh that users' employee receives, as reimbursement for their electricity use when charging at home.

**Translations:**
- DA: refusionspris
- DE: Rückerstattungswert
- EN: reimbursement price
- EN_US: reimbursement price
- ES: precio de reembolso
- FR: prix remboursé
- FR_CA: remboursement
- IT: prezzo di rimborso
- NL: vergoedingsprijs
- NO: refusjonspris
- RO: tarif rambursat
- SV: ersättningspris

---

## reliability
**Tag:** industry

**Description:** Measurement of how consistent, error-free operation of a charge point or network is, i.e. measured by uptime and availability. A key quality metric in the EV industry used to assess infrastructure performance

**Translations:**
- DA: driftstabilitet
- DE: Zuverlässigkeit
- EN: reliability
- EN_US: reliability
- ES: fiabilidad
- FR: fiabilité
- FR_CA: fiabilité
- IT: affidabilità
- NL: betrouwbaarheid
- NO: pålitelighet
- RO: fiabilitate
- SV: tillförlitlighet

---

## residental charging
**Tag:** industry

**Description:** Type of charging, accommodated in a residential area.

**Translations:**
- DA: opladning i boligforeninger
- DE: Laden zu Hause
- EN: residental charging
- EN_US: residential charging
- ES: carga residencial/carga en residencias
- FR: recharge à domicile
- FR_CA: recharge à domicile
- IT: ricarica residenziale
- NL: laden voor bewoners
- NO: ladeløsning for borettslag
- RO: încărcare într-o zonă rezidențială
- SV: bostadsladdning

---

## reverse charge
**Tag:** industry

**Description:** EU VAT regulation term. A mechanisms where buyer, not the seller, reports and pays the VAT, used in cross-border B2B sales. VAT is shown as 0% on invoices with a reverse charge note.

**Translations:**
- DA: omvendt betalingspligt        
- DE: Umkehr der Steuerschuldnerschaft (Reverse Charge)
- EN: reverse charge
- EN_US: no tax charged (EU reverse charge rule)
- ES: inversión del sujeto pasivo
- FR: autoliquidation
- FR_CA: taxe au preneur
- IT: inversione contabile        
- NL: BTW verlegd
- NO: omvendt avgiftsplikt
- RO: taxare inversă
- SV: omvänd betalningsskyldighet

---

## roadmap
**Tag:** industry

**Description:** A high-level overview of the software development process. Monta provides a Public Roadmap, displaying planned features, those in development, and those launched

**Translations:**
- DA: roadmap
- DE: Roadmap
- EN: roadmap
- EN_US: roadmap
- ES: plan de desarrollo
- FR: feuille de route
- FR_CA: feuille de route
- IT: roadmap
- NL: roadmap
- NO: roadmap
- RO: foaie de parcurs
- SV: färdplan

---

## roaming
**Tag:** industry

**Description:** Enables EV-drivers to find, access and pay for charging across multiple public networks.

**Translations:**
- DA: roaming
- DE: Roaming
- EN: roaming
- EN_US: roaming
- ES: roaming
- FR: itinérance
- FR_CA: itinérance
- IT: roaming
- NL: roaming
- NO: roaming
- RO: roaming
- SV: roaming

---

## sales tax
**Tag:** industry

**Description:** US-centric term - a tax  % imposed on the sale of goods and services at the point of purchase, paid by end-user.  In EU countries, the VAT (Value Added Tax) is the equivalent of the U.S. sales tax


**Translations:**
- DA: moms
- DE: USt
- EN: sales tax 
- EN_US: sales tax 
- ES: IVA
- FR: TVA
- FR_CA: taxes de vente
- IT: IVA
- NL: BTW
- NO: MVA
- RO: TVA
- SV: moms

---

## schedule
**Tag:** product

**Description:** Verb. used as CTA for a planning feature, allowing adjustment of the charging timeslot.

**Translations:**
- DA: planlægge
- DE: zeitplan
- EN: schedule
- EN_US: schedule
- ES: programar
- FR: planifier
- FR_CA: planifier
- IT: pianificare
- NL: schema
- NO: timeplan
- RO: programare
- SV: schemalägg

---

## scheduled charging
**Tag:** product

**Description:** Charge method: The ability to set a specific time or schedule for starting or completing the EV charging process

**Translations:**
- DA: planlagt opladning
- DE: Geplante Ladung
- EN: scheduled charging
- EN_US: scheduled charging
- ES: carga programada
- FR: recharge programmée
- FR_CA: recharge planifiée
- IT: ricarica programmata
- NL: geplande laadsessie
- NO: planlagt ladeøkt
- RO: încărcare programată
- SV: schemalagd laddning

---

## self-healing
**Tag:** industry

**Description:** Adjective, self-healing refers to systems or software that can automatically detect, diagnose, and resolve issues without human intervention

**Translations:**
- DA: self-healing
- DE: selbstheilend
- EN: self-healing
- EN_US: self-healing
- ES: autorreparación
- FR_CA: autoréparation
- IT: ripristino automatico
- NL: zelfherstellend
- NO: selvhelbrendende
- RO: auto-reparare
- SV: self-healing

---

## serial number
**Tag:** industry

**Description:** A unique identifier assigned to a charging device, used for tracking and management within a network.

**Translations:**
- DA: serienummer
- DE: Seriennummer
- EN: serial number
- EN_US: serial number
- ES: número de serie
- FR: numéro de série
- FR_CA: numéro de série
- IT: numero di serie
- NL: serienummer
- NO: serienummer
- RO: număr de serie
- SV: serienummer

---

## service provider
**Tag:** industry

**Description:** Stakeholder, that provides charging services (monthly subscriptions), e.g. charge point operator.

**Translations:**
- DA: serviceudbyder
- DE: Dienstleister
- EN: service provider
- EN_US: service provider
- ES: proveedor de servicios
- FR: fournisseur de service
- FR_CA: fournisseur de services
- IT: fornitore di servizi
- NL: dienstverlener
- NO: tjenestetilbyder
- RO: furnizor de servicii
- SV: tjänsteleverantör

---

## site
**Tag:** industry

**Description:** Refers to a charging site, often used synonymously. Use established translations for "charging site" when the source mentions "site* to reduce ambigiouty

**Translations:**
- DA: ladeplads, ladested
- DE: Standort
- EN: site
- EN_US: site
- ES: estación
- FR: station de recharge
- FR_CA: site
 site de recharge
- IT: stazione
- NL: locatie
- NO: ladested
- RO: locație
- SV: laddplats

---

## site owner
**Tag:** industry

**Description:** An entity that has installed charge points within their premises, e.g. parking lot owner.

**Translations:**
- DA: ladepladsejer
- DE: Standortbetreiber
- EN: site owner
- EN_US: site owner
- ES: dueño de la estación/propietario de la estación
- FR: propriétaire de station
- FR_CA: propriétaire de site
- IT: proprietario del sito
- NL: locatie-eigenaar
- NO: eier av ladested
- RO: proprietarul locației
- SV: laddstationsägare

---

## smart charge
**Tag:** industry

**Description:** A verb describing a charging method based on user preferences, such as cost, renewable energy, and CO2 footprint, when the 'SmartCharge* feature is enabled.

**Translations:**
- DA: smartlade
- DE: intelligent laden
- EN: smart charge
- EN_US: smart charge
- ES: cargar inteligentemente
- FR: recharger intelligemment
- FR_CA: recharge intelligente
- IT: ricaricare in modo intelligente
- NL: slim laden
- NO: smartlade
- RO:  încărcare inteligentă
- SV: ladda smart

---

## smart charging
**Tag:** industry

**Description:** Noun. Feature enabling to charge EV, when energy is at its lowest price or is greenest. Non-translatable.

**Translations:**
- DA: intelligent opladning
- DE: intelligentes Laden
- EN: smart charging
- EN_US: smart charging
- ES: carga inteligente
- FR: recharge intelligente
- FR_CA: recharge intelligente
- IT: ricarica intelligente
- NL: slim laden
- NO: smart lading
- RO: încărcare inteligentă
- SV: smart laddning

---

## solar charging 
**Tag:** industry

**Description:** B2C feature in mobile app, enables EV charging directly from solar panels, using real-time or forecasted production data. Helps maximise self-consumption, reduce grid reliance, lower energy costs, and minimise carbon footprint. Can be configured to operate only when surplus solar energy is available.

**Translations:**
- DA: solopladning
- DE: Solarladen
- EN: solar charging 
- EN_US: solar charging 
- ES: carga solar
- FR: recharge solaire
- FR_CA: recharge solaire
- IT: ricarica solare
- NL: zonneladen
- NO: sollading
- RO: Încărcare solară
- SV: solladdning

---

## solar surplus charging
**Tag:** industry

**Description:** The process of using excess solar energy generated by a photovoltaic (PV) system to charge an electric vehicle (EV)

**Translations:**
- DA: opladning med solenergioverskud
- DE: Laden mit Solarüberschuss
- EN: solar surplus charging
- EN_US: solar surplus charging
- ES: carga con excedente solar
- FR: recharge solaire avec excédent
- FR_CA: recharge solaire avec surplus
- IT: ricarica con surplus solare
- NL: opladen met zonne-energie overschot
- NO: lading med solenergioverskudd
- RO: încărcare cu surplus solar
- SV: laddning med solenergiöverskott

---

## sort code
**Tag:** industry

**Description:** A unique identifier of the individual branch or bank office where a bank account is held.

**Translations:**
- DA: reg. nr.
- DE: Bankleitzahl
- EN: sort code
- EN_US: routing number
- ES: Sort Code
- FR: code guichet
- FR_CA: numéro de routage
- IT: codice SORT
- NL: sorteercode
- NO: IBAN OR BIC/SWIFT
- RO: Cod SWIFT
- SV: clearingnummer

---

## spot price
**Tag:** product

**Description:** Spot price defines a master price based on Nordpool data (leading power market in Europe), accounting for electricity price fluctuations. When setting up a price group, a master price is selected per kWh, minute, or spot data. Available only in markets with Nordpool access.

**Translations:**
- DA: spotpris
- DE: Spotpreis
- EN: spot price
- EN_US: spot price
- ES: precio spot
- FR: prix spot
- FR_CA: prix au comptant
- IT: prezzo spot
- NL: spotprijs
- NO: spotpris
- RO: preț spot
- SV: spotpris

---

## start fee
**Tag:** product

**Description:** Fee description (visible to users) applied when charging starts. A one-off fee at the beginning of the charge. Used in the price group context, with four fee types in total: starting fee, charging fee, minute fee, and idle fee

**Translations:**
- DA: startgebyr
- DE: Startgebühr
- EN: starting fee
- EN_US: starting fee
- ES: tasa inicial
- FR: frais de départ
- FR_CA: frais de départ
- IT: costo iniziale
- NL: opstartkosten
- NO: startavgift
- RO: taxă de pornire
- SV: startavgift

---

## start time
**Tag:** product

**Description:** Start time of a charge session, used in the "Schedule" feature, one of the charge methods (Charge now, SmartCharge, and Schedule)

**Translations:**
- DA: starttidspunkt
- DE: Startzeit
- EN: start time
- EN_US: start time
- ES: hora de inicio
- FR: heure de début
- FR_CA: heure de début
- IT: ora di inizio
- NL: starttijd
- NO: starttid
- RO: ora de începere
- SV: starttid

---

## state of charge (SoC)
**Tag:** industry

**Description:** It refers to the current battery charge level as a percentage of its total capacity.

**Translations:**
- DA: ladetilstand
- DE: Ladestand
- EN: state of charge (SoC)
- EN_US: state of charge (SoC)
- ES: estado de la carga (SoC)
- FR: état de charge
- FR_CA: état de charge
- IT: stato della ricarica
- NL: laadstatus
- NO: ladetilstand
- RO: starea Încărcării (SoC)
- SV: laddningsstatus

---

## statement
**Tag:** industry

**Description:** An invoice overview, a total overview of the parts that makes up the invoice for a certain period of time.

**Translations:**
- DA: opgørelse
- DE: Rechnungsübersicht
- EN: statement
- EN_US: statement
- ES: extracto
- FR: relevé
- FR_CA: relevé
- IT: estratto
- NL: factuuroverzicht
- NO: fakturaoversikt
- RO: situație facturi
- SV: utdrag

---

## sticker
**Tag:** industry

**Description:**  QR code, enables access to the charging service.

**Translations:**
- DA: sticker
- DE: Sticker
- EN: sticker
- EN_US: sticker
- ES: sticker
- FR: sticker
- FR_CA: autocollant
- IT: sticker
- NL: sticker
- NO: klistremerke
- RO: autocolant
- SV: sticker

---

## support settings
**Tag:** product

**Description:** Settings under Team settings in Portal. Support settings are mapped out in 3 sub-categories, i.e. Monta, installer and operator to differentiate which support falls under which actor.

**Translations:**
- DA: supportindstillinger
- DE: Einstellungen
- EN: support settings
- EN_US: support settings
- ES: ajustes de soporte
- FR: paramètres de support
- FR_CA: paramètres de soutien
- IT: impostazioni di supporto
- NL: ondersteuningsinstellingen
- NO: støtteinstillinger
- RO: setări asistență
- SV: supportinställningar

---

## surplus energy
**Tag:** industry

**Description:** Excess electricity generated beyond immediate consumption needs, which can be stored, used for charging, or fed back into the grid.

**Translations:**
- DA: overskudsstrøm
- DE: Überschussstrom
- EN: surplus energy
- EN_US: surplus energy
- ES: electricidad excedente
- FR: électricité excédentaire
- FR_CA: surplus d’énergie
- IT: elettricità in eccesso
- NL: overtollige elektriciteit
- NO: overskuddstrøm
- RO: electricitate excedentară
- SV: överskottsel

---

## swipe
**Tag:** product

**Description:** Verb, CTA for approving and enabling certain task in the consumer app.

**Translations:**
- DA: swipe
- DE: swipe
- EN: swipe
- EN_US: swipe
- ES: aceptar/deslizar
- FR: balayer
- FR_CA: glisser
- IT: scorrere
- NL: swipe
- NO: sveip
- RO: glisare
- SV: svajpa

---

## tariff
**Tag:** industry

**Description:** See "Network tariff" for more details. Used both as stand-alone "tariff" and as "network tariff"

**Translations:**
- DA: tarif
- DE: Netzentgelt
- EN: tariff
- EN_US: tariff 
- ES: tarifa
- FR: tarif
- FR_CA: tarif
- IT: tariffa
- NL: tarief
- NO: tariff
- RO: tarif
- SV: elnätsavgift

---

## tax ID
**Tag:** industry

**Description:** A Tax Identification in the US,  a unique number assigned by the Internal Revenue Service (IRS) to individuals and entities for tax reporting purposes. European equivalent is VAT number

**Translations:**
- DA: see VAT number
- DE: See VAT number
- EN: Tax ID
- EN_US: see VAT number
- ES: see VAT number
- FR: see VAT number
- FR_CA: numéro de CA
- IT: see VAT number
- NL: see VAT number
- NO: see VAT number
- RO: see VAT number
- SV: see VAT number

---

## tax identification number 
**Tag:** industry

**Description:** Italian billing compliance term, used in Italy-only for fiscal activities. No equlavent concepts in other languages, thus do a literal translation. 

**Translations:**
- DA: skatteidentifikationsnummer
- DE: Steueridentifikationsnummer
- EN: tax identification number 
- EN_US: tax identification number 
- ES: número de identificación fiscal
- FR: numéro d'identification fiscale
- FR_CA: numéro d'identification fiscale
- IT: codice fiscale
- NL: fiscaal identificatienummer
- NO: skatteidentifikasjonsnummer
- RO: cod de identificare fiscală
- SV: skatteidentifikationsnummer

---

## team
**Tag:** brand

**Description:** Feature: A team manages charge points, members, prices, access, Wallet, and more. There are 4 team types: personal, private, professional, and operator.

**Translations:**
- DA: team
- DE: Team
- EN: team
- EN_US: team
- ES: equipo
- FR: groupe
- FR_CA: équipe
- IT: team
- NL: team
- NO: team
- RO: grup
- SV: team

---

## theme
**Tag:** product

**Description:** Functionality, enabling partners to brand Monta's software services with their branding elements.

**Translations:**
- DA: tema
- DE: Thema
- EN: theme
- EN_US: theme
- ES: tema
- FR: thème
- FR_CA: thème
- IT: tema
- NL: thema
- NO: tema
- RO: temă
- SV: tema

---

## transaction fee
**Tag:** product

**Description:** Monta retains 2 % of each transaction, occurring during a paid charge session.

**Translations:**
- DA: transaktionsgebyr
- DE: Transaktionsgebühr
- EN: transaction fee
- EN_US: transaction fee
- ES: tasa/cuota por transacción
- FR: frais de transaction
- FR_CA: frais de transaction
- IT: costo della transazione
- NL: transactiekosten
- NO: transaksjonsavgift
- RO: taxă tranzacție
- SV: transaktionsavgift

---

## user
**Tag:** product

**Description:** Concept - users in a general sense and encompasses all segments groups within b2c, b2b, b2p.

**Translations:**
- DA: bruger
- DE: Nutzer
- EN: user
- EN_US: user
- ES: usuario
- FR: utilisateur
- FR_CA: utilisateur
- IT: utente
- NL: gebruiker
- NO: bruker
- RO: utilizator
- SV: användare

---

## user role
**Tag:** product

**Description:** Role of the user in a team in the portal (e.g. Member, Administrator, ..)

**Translations:**
- DA: brugerrolle
- DE: Nutzerrolle
- EN: user role
- EN_US: user role
- ES: rol de usuario
- FR: rôle utilisateur
- FR_CA: rôle d’utilisateur
- IT: ruolo dell'utente
- NL: gebruikersrol
- NO: brukerrolle
- RO: rol utilizator
- SV: användarroll

---

## wallet
**Tag:** brand

**Description:** Monta's financial service feature, which facilitates transactions to and from bank accounts, payments, reimbursements etc.

**Translations:**
- DA: wallet
- DE: das Wallet
- EN: wallet
- EN_US: wallet
- ES: wallet
- FR: wallet
- FR_CA: portefeuille
- IT: portafoglio
- NL: wallet
- NO: wallet
- RO: wallet
- SV: wallet

---

## white label
**Tag:** industry

**Description:** Service, where Monta resells tech solutions to another company. The buying company rebrands the service as their own. 

**Translations:**
- DA: white label
- DE: White label
- EN: white label
- EN_US: white label
- ES: marca blanca
- FR: marque blanche
- FR_CA: marque blanche
- IT: white label
- NL: white label
- NO: white label
- RO: white label
- SV: white label

---

## wholesaler
**Tag:** industry

**Description:** Entities that specialize in selling EV charge points in bulk or on a wholesale level.

**Translations:**
- DA: grossist
- DE: Großhändler
- EN: wholesaler
- EN_US: wholesaler
- ES: mayorista
- FR: revendeur
- FR_CA: grossiste
- IT: rivenditore
- NL: groothandel
- NO: grossist
- RO: distribuitor
- SV: grossist

---

## workplace charging
**Tag:** industry

**Description:** Refers to charging accommodated at a work place

**Translations:**
- DA: opladning på arbejdspladsen
- DE: Laden am Arbeitsplatz
- EN: workplace charging
- EN_US: workplace charging
- ES: carga en el lugar de trabajo
- FR: recharge sur le lieu de travail
- FR_CA: recharge au travail
- IT: ricarica sul posto di lavoro
- NL: werkladen
- NO: lading på arbeidsplassen
- RO: încărcare la locul de muncă
- SV: laddning på arbetsplatsen

---
